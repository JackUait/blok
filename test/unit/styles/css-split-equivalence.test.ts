...
describe('main.css split — cascade-preserving equivalence', () => {
  ...
  it('total local CSS byte size stays within +2% of the pre-split baseline', () => {
    ...
    // Find in page: find.css (window-fixed top-layer bar, drag grip, match
    // map, replace row, lens, reduced motion) plus its palette tokens in
    // colors.css add 14,934 bytes; retain 200 bytes of headroom.
    const FIND_IN_PAGE_BYTES = 15_134;
