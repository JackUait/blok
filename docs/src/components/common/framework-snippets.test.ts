...
describe('framework-snippets', () => {
  it('provides a quick-start snippet set for every framework', () => {
    for (const id of FRAMEWORK_IDS) {
      ...
      expect(QUICK_START_SNIPPETS[id].create.code).toContain('@blok/core');
  ...
  it('imports the matching adapter entry point per framework', () => {
    expect(QUICK_START_SNIPPETS.react.create.code).toContain('@blok/react');
    expect(QUICK_START_SNIPPETS.vue.create.code).toContain('@blok/vue');
    expect(QUICK_START_SNIPPETS.angular.create.code).toContain('@blok/angular');
