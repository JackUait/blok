 ...
 *
 * Dynamic imports in production builds get transformed by Rollup into
 * chunk-loading chains like:
 *   const { clean: l } = await import("./chunk.mjs").then((c) => c.aX)
 *
 * When the chunk's re-export ordinal (e.g. `aX`) becomes stale due to
 * browser caching, CDN mismatch, or downstream re-bundling, `c.aX`
 * resolves to `undefined` and destructuring throws:
 *   TypeError: Cannot destructure property 'clean' of '(intermediate value)' as it is undefined
 *
 * @see https://github.com/JackUait/blok/issues/XX
 */
