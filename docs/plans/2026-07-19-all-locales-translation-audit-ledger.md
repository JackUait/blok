...
| `F-az-087` | `az` | `tools.database.duplicateView` | action register / caller context | `"Dublikat"` | `"Dublikat et"` | `database-tab-bar.ts` renders this as the live menu command that duplicates a view. The current noun names a duplicate; the bare imperative matches the selected compact-action register and sibling duplication commands. The focused regression and structural check pass after correction. | verified |
