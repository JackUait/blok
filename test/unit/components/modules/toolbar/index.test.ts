...
describe('Plus button interactions', () => {
  ...
  describe('read-only toggle behavior', () => {
    ...
    it('re-binds the settings toggler tooltip when toggleReadOnly is called', () => {
      const settingsTogglerHandler = (toolbar as unknown as {
        settingsTogglerHandler: { refreshTooltip: () => void };
      }).settingsTogglerHandler;
      const refreshTooltipSpy = vi.spyOn(settingsTogglerHandler, 'refreshTooltip');
  ...
  describe('read-only menu shortcut (Cmd/Ctrl + Slash)', () => {
    /**
     * Grabs the document-level keydown handler registered by enableModuleBindings
     */
    const getDocumentKeydownHandler = (): ((event: KeyboardEvent) => void) => {
      const listeners = (toolbar as unknown as { readOnlyMutableListeners: MutableListeners })
        .readOnlyMutableListeners;
      const call = (listeners.on as Mock).mock.calls.find(
        ([target, type]) => target === document && type === 'keydown'
      );

      if (!call) {
        throw new Error('document keydown handler was not registered');
      }

      return call[2] as (event: KeyboardEvent) => void;
    };

    const commandSlash = (): KeyboardEvent => new KeyboardEvent('keydown', {
      code: 'Slash',
      metaKey: true,
      cancelable: true,
    });

    let hoveredBlock: unknown;
    let openSpy: Mock;

    beforeEach(() => {
      (toolbar as unknown as { enableModuleBindings: () => void }).enableModuleBindings();

      hoveredBlock = { id: 'hovered-block', name: 'paragraph', holder: document.createElement('div') };
      openSpy = vi.fn();

      const blok = getBlok();

      (blok.BlockSettings as unknown as { open: Mock }).open = openSpy;
      (blok as unknown as { DragManager: { cancelTracking: Mock } }).DragManager = {
        cancelTracking: vi.fn(),
      };
    });

    it('opens block settings for the hovered block in read-only mode', () => {
      (getBlok().ReadOnly as unknown as { isEnabled: boolean }).isEnabled = true;
      (toolbar as unknown as {
        settingsTogglerHandler: { setHoveredBlock: (block: unknown) => void };
      }).settingsTogglerHandler.setHoveredBlock(hoveredBlock);

      const event = commandSlash();

      getDocumentKeydownHandler()(event);

      expect(openSpy).toHaveBeenCalledOnce();
      expect(openSpy.mock.calls[0]?.[0]).toBe(hoveredBlock);
      expect(event.defaultPrevented).toBe(true);
    });

    it('does nothing in read-only mode when no block is hovered', () => {
      (getBlok().ReadOnly as unknown as { isEnabled: boolean }).isEnabled = true;
    ...
    it('leaves the shortcut to BlockEvents when read-only is disabled', () => {
      (toolbar as unknown as {
        settingsTogglerHandler: { setHoveredBlock: (block: unknown) => void };
      }).settingsTogglerHandler.setHoveredBlock(hoveredBlock);
