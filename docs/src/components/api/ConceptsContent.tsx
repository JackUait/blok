  ...
  return (
    <div className="flex flex-col gap-12">
      ...
      <div className="flex flex-col gap-4">
        <h2 className={headingClass}>{t("api.concepts.shape.title")}</h2>
      ...
      {/* Blocks form a tree */}
      <div className="flex flex-col gap-4">
        <h2 className={headingClass}>{t("api.concepts.tree.title")}</h2>
      ...
      <div className="flex flex-col gap-4">
        <h2 className={headingClass}>{t("api.concepts.everywhere.title")}</h2>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr>
                ...
                <th className={thClass}>
                  {t("api.concepts.everywhere.whyHeader")}
                </th>
              </tr>
            </thead>
            <tbody>
              {exampleRows.map((row) => (
                <tr key={row} className="border-t border-border">
                  <td className={`${tdClass} whitespace-nowrap font-medium text-foreground`}>
                    {t(`api.concepts.everywhere.rows.${row}.thing`)}
                  </td>
      ...
      {/* What is not a block */}
      <div className="flex flex-col gap-4">
        <h2 className={headingClass}>{t("api.concepts.notBlocks.title")}</h2>
        <p className={proseClass}>
      ...
      <div className="flex flex-col gap-4">
        <h2 className={headingClass}>{t("api.concepts.why.title")}</h2>
          ...
          {whyItems.map((item) => (
            ...
            >
              <p className="font-display text-sm font-bold tracking-tight text-foreground">
                {t(`api.concepts.why.items.${item}.label`)}
              </p>
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-primary/20 bg-primary/[0.04] px-6 py-6">
        <h2 className={headingClass}>{t("api.concepts.decision.title")}</h2>
        <p className={proseClass}>
