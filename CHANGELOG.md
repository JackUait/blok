...
- **`@bloklabs/server`** — One shared C# implementation now ships in two forms: install `Blok.Server.AspNetCore` to register it inside an existing ASP.NET Core app with `AddBlokServer()`, `UseAuthorization<T>()` and `MapBlokServer()`, or run the same handlers through `npx @bloklabs/server` and `ghcr.io/jackuait/blok-server`. `POST /upload` takes a file, `POST /upload-by-url` re-hosts a pasted remote URL, and `GET /unfurl` returns the OpenGraph / Twitter-card metadata a bookmark card needs. Both consumer-supplied URL routes share one guarded outbound client, with S3 as the sole configured-endpoint exception. Local and S3-compatible storage, ticket/CORS/rate-limit ordering, archive names, flags, environment variables and response wires stay compatible across the NuGet and standalone forms. The npm wrapper now downloads the self-contained C# host for six OS/architecture targets and verifies it with `checksums.txt`. The current packages still store no documents and do not include database-block or MySQL integration; that work follows the delivery migration.
...
- **Release and CI wiring** — Nothing in this repository iterates workspaces automatically, so the package family remains pinned across version lockstep, publish lists, build/test graphs, docs verification and metadata laws. Server delivery now starts from a draft GitHub release: the tag workflow tests and formats the .NET 10 solution, packs `Blok.Server` and `Blok.Server.AspNetCore`, proves the produced NuGets in an isolated consumer, builds six self-contained host archives plus checksums, smoke-tests the native archive and linux/amd64 image, publishes NuGet/GitHub/GHCR outputs, verifies each is observable, and only then publishes the draft. The docs release event therefore cannot advertise an npm package, NuGet, archive or image that is still missing. Final server CI runs the full .NET solution and format gate, packed-package proof, the C# 58-case conformance harness, and focused artifact and delivery laws. The frozen Go oracle and its source, build, and release wiring were removed after the unwaived dual-target gate passed. Drift laws still fingerprint the hand-copied `@bloklabs/presets` declarations against their sources, so a mirrored type cannot silently disappear from the published `.d.ts`.
