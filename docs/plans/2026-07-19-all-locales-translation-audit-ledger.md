...
| `en` | English | Latin | ltr | concise US English; sentence-case UI | — | — | pending | pending | pending | `F-en-001`–`F-en-062` | pending |
...
| `F-global-001` | all non-English | 62 changed English source keys | source dependency | Localized values have not been re-reviewed against the 56 corrected source values and six new keys. | Re-audit all 62 dependent values in all 68 localized dictionaries and correct them where required. | English-source changes invalidate dependent semantic evidence; every complete locale pass must inspect all 62 dependencies, including `toolNames.clearFormat`, the expanded emoji categories, the 11 newly corrected contracts, both new media-caption placeholders, and all three localized notifier-dialog defaults. | open |
