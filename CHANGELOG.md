...
- **Translate a document without handing a model its JSON** — `extractTexts` and `injectTexts` move the prose past a model.
...
- **Markdown out of a saved document** — `blocksToMarkdown` is the synchronous, DOM-free twin of `markdownToBlocks`.
...
- **A heading keeps the anchor its links point at** — `HeaderData.anchor` renders as the heading's `id`.
...
- **Change a live document from outside the browser** — `POST /sync/{doc}/edit` and `/reset` work over HTTP.
...
- **Block controls on either side** — `config.toolbarPosition: 'left' | 'right'` moves the plus button and drag handle.
...
- **Framework adapters** — `useBlocks` takes `{ within: blockId }`, so a container re-renders only for its own subtree.
