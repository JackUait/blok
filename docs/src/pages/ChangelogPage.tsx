      ...
      <>
        ...
        <main className="changelog-main">
          <div className="changelog-hero">
            ...
            <h1 className="changelog-hero-title">Changelog</h1>
            <p className="changelog-hero-description">Error loading changelog: {error}</p>
          </div>
        </main>
      </>
    );
            ...
            {releases.map((release) => (
              ...
              >
                <div className="changelog-version">
                  {release.releaseUrl ? (
                    <a href={release.releaseUrl} className="changelog-version-number" target="_blank" rel="noopener noreferrer">
                      v{release.version}
                    </a>
                  ) : (
                    <span className="changelog-version-number">v{release.version}</span>
                  )}
                  <span className="changelog-version-type">{release.releaseType}</span>
                  <span className="changelog-version-date">{formatDate(release.date)}</span>
                </div>
                  ...
                  <div className="changelog-changes">
                    {release.changes.map((change, index) => (
                      <div key={index} className="changelog-change">
                        <span className={`changelog-change-badge ${change.category}`}>
                        ...
                        <span className="changelog-change-description">
                          {change.description}
                          {change.link && (
                            <Link to={change.link} className="changelog-change-link">
                              View docs
                              <Icons.external />
                            </Link>
                          )}
