/**
 * Builds the Angular adapter (`@blok/angular`) into an Angular Package
 ...
 *
 * 2. The adapter depends only on the PUBLIC `@blok/core` surface. Pointing
 ...
 *    exempt from the rootDir check) and map `@blok/core` to it. The runtime
...
// 1. Flatten the public `@blok/core` types into a single self-contained
...
// repo-root `types/` or `markdown/types` to `'@blok/core'` (the staged
...
// `@blok/core` exports, so we append them to `blok-core.d.ts` first.
...
// Stub `@blok/core/markdown` so the dynamic `await import('../../markdown/index')`
...
writeFileSync(
  ...
  [
    `import type { OutputBlockData, MarkdownImportConfig } from '@blok/core';`,
 ...
 *  `@blok/core` alias (`blok-core.d.ts`) instead.  Also rewrites the
 *  markdown helpers so they stay resolvable in the staging layout.
 */
const rewriteTypeImports = (filePath) => {
  let src = readFileSync(filePath, 'utf8');

  // 1. `import type { … } from '…/types/…'` or `'…/markdown/types'` → @blok/core
  const TYPE_PATTERNS = [
  ...
  if (symbols.length > 0) {
    ...
    src = `import type { ${unique.join(', ')} } from '@blok/core';\n` + src;
  }

  // 2. Dynamic `import('…/markdown/index')` → `import('@blok/core/markdown')`
  //    so the path is resolvable under the staging tsconfig's `paths` aliases.
  src = src.replace(
    /import\(['"](?:\.\.\/)*markdown\/index['"]\)/g,
    `import('@blok/core/markdown')`
  );
...
for (const tsFile of gatherTsSourceFiles(stagingDir)) {
  ...
  // '/types' is a leaked reference that should have been rewritten to '@blok/core'.
  const re = /from\s+['"]((?:\.\.\/)+[^'"]+)['"]/g;
  let m;
  while ((m = re.exec(src)) !== null) {
    ...
    if (
      ...
      throw new Error(
        ...
        `'${spec}' was not rewritten to '@blok/core'. ` +
  ...
  JSON.stringify(
    {
      ...
      compilerOptions: {
        ...
        baseUrl: '.',
        paths: {
          '@blok/core': ['./blok-core.d.ts'],
          '@blok/core/markdown': ['./blok-markdown.d.ts'],
        },
      },
