...
- **Gates and dependencies** — The .NET solution keeps three test layers plus cross-runtime conformance and package smoke tests, and CI requires 80% line and branch coverage on merged production code, runs the SDK analyzers with warnings as errors, audits direct and transitive NuGet packages, scans for committed secrets, scans the server tree and the built image, and analyzes C# with CodeQL. CI gates were parallelized to fit a six-minute budget. Root, docs, .NET, Docker and GitHub Actions dependencies were upgraded across the release.
