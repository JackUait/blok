...
describe('Paste module', () => {
  ...
  describe('HtmlHandler', () => {
    it('converts Google Docs style spans to semantic tags before sanitization', async () => {
      const { paste, mocks } = createPaste();
      ...
      // Mock clean to pass through HTML (simulating that b/i are allowed by sanitizer)
      ...
      const html = '<div><span style="font-weight:700">bold text</span> and <span style="font-style:italic">italic text</span></div>';
      ...
      expect(detail.data.innerHTML).toContain('<b>bold text</b>');
      expect(detail.data.innerHTML).toContain('<i>italic text</i>');
    });

    it('converts Google Docs bold+italic combo span to nested semantic tags', async () => {
      const { paste, mocks } = createPaste();
      ...
      const html = '<div><span style="font-weight:700;font-style:italic">bold italic</span></div>';
