...
public sealed class BlokServerRegistrationTests
{
  ...
  [Fact]
  public void RegistersACustomCollabOperationStore()
  {
    var services = new ServiceCollection();
    ...
    services
        ...
        .UseCollabOperationStore<StubCollabOperationStore>();

    var descriptor = Assert.Single(
        services,
        candidate => candidate.ServiceType == typeof(ICollabOperationStore));
    Assert.Equal(typeof(StubCollabOperationStore), descriptor.ImplementationType);
  ...
  {
    var services = new ServiceCollection();
    var builder = services.AddBlokServer(options =>
    {
      options.CollabEnabled = true;
      options.DocEndpoint = "https://app.example.com/api/blok-docs";
      options.CollabDirectory = "/srv/blok/collab";
    });
    var descriptorsBefore = services.Count;

    builder.UseCollabOperationStore<StubCollabOperationStore>();
    builder.UseCollabOperationStore<SecondStubCollabOperationStore>();

    // Net one descriptor: the second call replaces the first instead of
    // stacking, exactly as UseAuthorization<T> does.
    Assert.Equal(descriptorsBefore + 1, services.Count);
    var descriptor = Assert.Single(
        services,
        candidate => candidate.ServiceType == typeof(ICollabOperationStore));
    Assert.Equal(
        typeof(SecondStubCollabOperationStore),
        descriptor.ImplementationType);
    Assert.Single(
        services,
        candidate => candidate.ServiceType == typeof(ICollabWorkingSetStore));
    Assert.Single(
        services,
        candidate => candidate.ServiceType == typeof(ICollabRoomManager));
    Assert.Single(
        services,
        candidate => candidate.ServiceType == typeof(IBlobStore));
  }
  ...
  private sealed class StubCollabOperationStore : ICollabOperationStore
  {
