...
describe('TrustedBy', () => {
  ...
  // These cards are deliberately static: no hover lift (movement or shadow) and
  // no touch-press lift. They opt out of the shared bento-tile hover rise via
  // `.bento-tile--no-lift`, and a finger press must NOT toggle `.is-touch-active`
  // (which the shared CSS reads to deepen the shadow). The inner links stay
  // tappable regardless.
  describe('static cards (no hover or touch lift)', () => {
    it('opts both cards out of the bento-tile hover lift', () => {
      renderTrustedBy();
      const featured = screen.getByTestId('trusted-featured');
      const side = screen.getByTestId('trusted-stats').closest('.bento-tile');
      expect(featured).toHaveClass('bento-tile--no-lift');
      expect(side).not.toBeNull();
      expect(side!.classList.contains('bento-tile--no-lift')).toBe(true);
    });

    it('does not light the featured card on touch press', () => {
      renderTrustedBy();
      const card = screen.getByTestId('trusted-featured');
    ...
    it('does not light the stats/contact card on touch press', () => {
      renderTrustedBy();
