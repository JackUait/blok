// src/vue/registry-map.ts
import type { BlockPortalRegistry } from './block-portal-registry';

/**
 * Associates each live Blok instance with its portal registry, keyed by the RAW
 * editor (mirrors the holder WeakMap). `useBlok` sets it at creation; `BlokContent`
 * reads it to mount the shared `BlockPortalHost`. WeakMap → no leak on destroy.
 */
