...
export const serverLimits: ServerLimit[] = [
  ...
  },
  {
    ...
    body: 'Two promises about POST /sync/{doc}/edit hold only where the service has an operation journal: retrying the same Blok-Idempotency-Key returns the first result instead of applying the edit a second time, and reusing that key for different work is refused with 409. No flag turns a journal on, and the working copy is not one — neither the folder named by --collab-dir nor the prefix named by --collab-s3-prefix keeps the history of what it holds. The journal is a seam you fill: inside your own ASP.NET Core app, implement ICollabOperationStore over storage you already trust with transactions, and register it with AddBlokServer(...).UseCollabOperationStore<YourStore>(). Without one the service is working-copy-only, and an outside edit behaves like any other request you can retry: it is applied, answered 204, and applied again if you send it twice. With one, every accepted change is committed to your journal before anybody is told about it, so a change that reached a tab is a change that survived the process dying — and an edit the journal cannot commit is answered 503 with nothing relayed, instead of a 204 for a save that did not happen. What the service asks of that implementation is strict: one live writer per document, a read that sees every earlier commit, durable before the call returns, the duplicate check and the sequence assignment in one step, and any failure thrown rather than swallowed. The server package’s README states the whole list beside a working registration.',
  },
  {
    id: 'collab-rollback-boundary',
    title: 'Turning the operation journal off is not a rollback',
    body: 'Registering an operation store is close to one-way per document. A journal-backed document keeps no working copy at all, and the whole-JSON record your endpoint holds is only refreshed by a published checkpoint, an eviction and a drain — never once per edit window. Read that as a ceiling rather than a promise: the check below is what tells you how fresh the record actually is. Between those moments the journal is ahead of everything else, so the last edits the service acknowledged sit nowhere a build without your store can read. Unregister the store — or go back to a binary that never had it — and the document lands on whatever else it has. A document that has been journal-backed from the start has no working copy, so it is loaded fresh from your endpoint and comes back as the last record that endpoint accepted; every operation acknowledged since then is still in your journal, and nothing serves it. A document that ran under --collab-dir or --collab-s3-prefix before you registered the store still holds the working copy it had that day. A working copy with anything in it wins the open and your endpoint is never consulted, so that document comes back as it was on the day you switched — with no error and nothing in the log to say so. Blok does not keep a second whole-document copy beside the journal to make the switch back instant: the journal is the record, and the JSON is a projection of it. Run this before you roll back. Drain the service first, which flushes every open room’s record — the standalone host drains on a graceful stop, and an in-process app calls ICollabRoomManager.DrainAsync. Then check, per document, that the last write-back your endpoint accepted carries a Blok-Doc-Sequence equal to your journal head’s durable-through on the same Blok-Doc-Lineage; anything short of that is what the rollback drops. For every document that comes up short, open it once on the new build — a read-only join is enough — and run the drain and the check again: loading a journal-backed room marks its record owed, and a document whose process crashed with one owed had no room for the drain to flush. Only then start the old build, and keep clients held off while you do. A room hydrated from a working copy carries no document version yet, so its first write-back goes out with no Blok-Doc-Version and your endpoint has nothing to answer 409 on; one character typed by one user before the reset lands writes the day-of-switch document over the record you just verified. Finish with one POST /sync/{doc}/reset on it for every document that had a working copy before you registered the store: that empties the copy, and the next open seeds from your endpoint. Let clients back in after the last reset has returned, not before. The way forward is one-way too — register the store again and the journal wins the open, so whatever was typed while the old build was serving is not in it.',
  },
  {
    id: 'collab-reset',
    title: 'Changing a document from outside: two calls, and when to use which',
    body: 'While a document is open for live collaboration, the working copy wins: a change written straight into your own records will be overwritten the next time the service writes the document back. So there are two ways in from the outside, and both take a write pass for that document. For a change to a few blocks — a status flipped by a workflow, a field filled in by a job — POST /sync/{doc}/edit with a list of what to insert, update or remove; the change lands in the document, appears in everybody’s tab within moments, and gets written back to your records like any other edit. The whole request is one step: if any part of it is refused, nothing is applied. Every edit also needs a `Blok-Idempotency-Key` header with 1 to 128 printable ASCII characters. With an operation journal, retrying the same key with the same request returns the first result without applying it again; reusing it for different work returns 409. A 204 follows the durable commit and includes `Blok-Doc-Lineage` and `Blok-Doc-Sequence` response headers. If that commit cannot finish, the service returns 503 and does not relay the change. A working-copy-only service does not deduplicate the key and never answers 409 for reusing one: retrying it has ordinary retry behavior, and its 204 still starts the existing write-back retry path, so your own records trail by the write-back delay either way. Two refusals to plan for, both 422 with nothing applied: placing a block under a parent that has no children list, and an `after` the parent’s children list does not hold. Each answer names the step and the block: `collab: op 0: block "p1" has no children list, so nothing can be placed under it.` and `collab: op 0: block "b7" is not in the document order, so nothing can be placed after it.` Inserts apply one after another against the document as it stands, so two inserts naming the same `after` land in reverse order — chain them, each naming the one before it, to append in order. For replacing a document wholesale — a migration, a support fix — save your version to your own records first, then call POST /sync/{doc}/reset: the service throws its working copy away, loads the document fresh from your endpoint, and tells every open tab to drop what it had and pick up the new version. Nobody has to close anything either way. Editing a document nobody has open works too; the service loads it for the occasion. One more thing closes the gap between your save and the reset: once your endpoint answers with a version, every write-back carries the last one it gave in a Blok-Doc-Version header, and that header is your optimistic-concurrency handle. Bump the version on your own save and answer a stale PUT with 409; the service treats it as a failed write-back and retries, and stays refused until the reset loads your version, so a write already on the wire when you save cannot land over it.',
  },
  {
  ...
  },
  {
    ...
    body: 'A document opens read-only and stays that way until the first sync arrives: the editor never starts from a copy of its own, because two browsers each starting from their own copy and then syncing would duplicate every block. Once that first sync lands the document becomes editable — as long as the pass grants write and your app has not put the editor in read-only itself. After that the behaviour is deliberately asymmetric: if the connection drops, the document stays editable. What gets typed piles up in that tab and is shipped the moment it reconnects, so a tunnel or a sleeping laptop does not freeze the page under someone’s hands. The editor reports each step on the collaboration:status event — connecting, connected, offline while it keeps retrying, or error when it has stopped for good and will not reconnect by itself — with the list of people present, so you can render your own indicator in the shape your product already uses. Blok does not paint one for you. None of those four says the work is saved: they describe the socket, not the queue behind it. The next entry is the one that says what has been taken.',
  },
  {
    id: 'collab-save-state',
    title: 'Connected is not saved: the editor reports the two separately',
    body: 'Connecting, connected and offline describe the socket, and none of them says the work has been taken. The same collaboration:status event carries a second block that does: save. Its state is one of five. saved — nothing is waiting. pending — edits are queued in this tab and not acknowledged yet, which a connected session reports too, because a socket that is up is not a save. blocked — the local queue broke and nothing more may be sent, with the reason local-storage-failed; editing stops for the rest of the session. quarantined — edits were taken out of the queue and will never be sent, and quarantinedOperations counts them; with the offline option on they stay in this browser’s copy, and without it only that count survives. unavailable — no durable save can be reported at all, which is what a service without an operation journal answers, with the reason legacy-protocol once it has said so. Beside the state, pendingOperations and pendingBytes size the queue, and serverSequence is where the journal put the last acknowledged edit — a decimal string, because the ceiling does not fit a JavaScript number. Draw connected with pending as saving, saved as saved, quarantined as an error somebody has to be told about, and unavailable as no save indicator at all rather than a green one. The event carries this whether or not the connection changed: a save state that moves on its own publishes one, an identical payload publishes nothing, and one replay lands shortly after the editor is ready so a listener attached late still learns where it stands. One caution about the counts: a local queue that cannot be read at all reports zero rows, so zero under blocked means unknown rather than nothing waiting. The boundary is exact: an edit counts as saved once the service has committed it to your operation journal, and that append has to return before anybody else is shown the edit. A socket send is not it, a peer seeing the change is not it, and neither is the whole-JSON write-back to your endpoint, which happens later and can fail on its own. This is also why one person on their own should use it: a single-user editor pointed at the service gets the same queue, the same journal and the same receipt as ten people do, and save is what an indicator should read. Without an operation journal none of it is reachable — no flag registers one, so a stock service negotiates the legacy protocol and reports unavailable for the life of every session.',
  },
  {
    id: 'collab-offline-reload',
    title: 'A reload while it says offline loses what was typed offline',
    body: 'By default those offline edits live in that tab and nowhere else until it reconnects: reload, close it or lose the browser before that happens and they are gone, because the tab was holding the only copy and nothing had reached the working copy or your records yet. Turning on the editor’s offline option changes that — it keeps a copy of the document in that browser, so a reload picks the work back up and sends it on the next connection. It is off unless you ask for it, because it writes document content into the visitor’s browser storage, which is a decision about your data rather than about the editor. Two things to know about the copy: nothing is kept until the first successful sync, so an editor that never reached the service still cannot start from a copy of its own; and the copy is thrown away whenever the service says its history no longer matches — after a reset, for instance — so a stale copy can never be mistaken for the live document. Turning it on requires one more thing: an offlineScope, a stable id for whoever is signed in. Browser storage belongs to the browser rather than to a person, and that id is what keeps each person’s copy to themselves — the editor refuses to start with the offline option on and no offlineScope, so there is no unpartitioned copy left to get wrong. Make that id opaque and stable — the same string every time that person signs in, on every device. Anything that rotates, a ticket or a session id, mints a fresh partition on every refresh and strands every copy behind it, offline edits included. The copy is also not a backup: Blok never asks the browser to keep it, so a browser short of space may drop it without telling anyone, and clearing site data does the same on purpose. Without the offline option, the guidance stays the plain one: while the editor says it is offline, leave the tab alone and let it reconnect.',
  },
  {
