...
editor.destroy();`,
      },
      {
        ...
        returnType: "void",
        description:
          "Installs, replaces or removes the live editor callbacks in place: `onChange`, `onSave`, `onEnter`, `onSubmit`, `onBeforeRender`, `onAfterRender`. Because the work happens in place, caret, selection, scroll and undo history all survive.\n\nOnly the keys you pass are touched. A key whose value is `undefined` UNSETS that handler. That matters because the presence of a callback is itself the semantics.\n\n- An `onSubmit` makes Enter serialize and submit instead of splitting the block.\n- An `onSave` arms the change-observation pipeline.\n\nUse it to make a callback reactive without recreating the editor. The React, Vue and Angular adapters drive this setter for you when a prop, listener or `[config]` callback appears or disappears.",
        params: [
          {
            name: "handlers",
            type: "LiveHandlers",
...
editor.handlers.set({ onSave: undefined });`,
      },
      {
        ...
        returnType: "Promise<void>",
        description:
          "Static method. It resolves once every Blok instance in scope has finished booting, which means each instance's `isReady` has settled. A rejection counts as settled. It is a collective readiness signal for pages that host several instances, and it replaces hand-aggregated per-instance `onReady` callbacks.\n\n- Pass `within` (an Element) to count only instances mounted inside a subtree you own.\n  - Then an unrelated editor elsewhere on the page cannot hold your gate closed.\n- Pass `settleOn: 'rendered'` to extend readiness from construction to content in the DOM, which also covers re-renders from `render(data)` after boot.\n\nAn empty scope resolves right away. Instances that appear while the promise is pending extend the wait. Instances constructed after it resolves are not covered, so call it again, or use `subscribeReady()` for a live signal.",
...
composer.focus();`,
      },
      {
        ...
        description:
          "Static method. It gives a synchronous readiness snapshot for a scope.\n\nThe snapshot says how many instances match `within`. It also says how many are still pending at the requested `settleOn` depth, and whether the scope is settled. An empty scope reports `ready: true`, so you do not need a \"nothing to wait for\" special case.",
...
unsubscribe();`,
      },
      {
        ...
        returnType: "string",
        description:
          "A named export of the package root, not a member of the Blok class. It builds a CSS selector from a `DATA_ATTR` value.\n\n- With no `value` it produces a presence selector.\n- With one it produces an equality selector.\n\nUse it together with `Blok.DATA_ATTR` instead of matching Blok's internal class names, which are not part of the public surface.",
...
document.querySelectorAll(createSelector(DATA_ATTR.selected, true));`,
      },
      {
        ...
        returnType: "string",
        description:
          "Named exports of the `@bloklabs/core/icons` subpath, not members of the Blok class. They are Blok's own glyphs as SVG strings, one `Icon*` constant per glyph (`IconBold`, `IconPlus`, `IconTrash`, `IconWarning`, …).\n\nThey are plain strings, so they drop straight into the places a tool must supply markup: a tool's `static get toolbox()` icon and the entries returned by `renderSettings()`. The subpath ships a generated, self-contained declaration file (`types/icons.d.ts`) that lists every constant, so editor autocomplete is the reference for the full set.",
    ...
    properties: [
      {
        ...
        description:
          "A named export of the package root (`import { DATA_ATTR } from '@bloklabs/core'`), not a property of the editor instance. It holds the stable `data-blok-*` attribute names Blok writes on its DOM.\n\nThis is the supported way to query editor DOM and write host tests, instead of matching internal class names. The `DataAttrKey` / `DataAttrValue` types and the `createSelector()` helper are exported alongside it.",
      },
      {
        name: "BLOK_FONT_SIZE_TOKENS",
        type: "BlokFontSizeTokens",
        description:
          "A named export of the package root (`import { BLOK_FONT_SIZE_TOKENS } from '@bloklabs/core'`), not a property of the editor instance. It gives the CSS custom property each `style.fontSize` scenario writes, in a map shaped exactly like the config itself (`BLOK_FONT_SIZE_TOKENS.paragraph`, `.heading[1]`, `.list.checklist`, `.bookmark.link`, …).\n\nUse it wherever typography is driven by a channel other than the constructor config: a per-region CSS rule, or `editor.tokens.set({ [BLOK_FONT_SIZE_TOKENS.paragraph]: '18px' })` at runtime. Then the custom property names never have to be hand-copied, and a rename is a compile error instead of a silent no-op.",
      },
      {
        name: "version",
        type: "string",
      ...
      },
      {
        ...
        description:
          "A type exported from the package root (`import type { PendingBlok } from '@bloklabs/core'`), not a property of the editor instance.\n\nIt is the surface guaranteed to exist synchronously between `new Blok(config)` and `isReady` resolving. Blok builds its module APIs (`blocks`, `caret`, `history`, `readOnly`, …) asynchronously, so reading them earlier returns `undefined`.\n\n`PendingBlok` declares only the eight members listed here. That turns the window into a compile error instead of an `undefined` at runtime: `const pending: PendingBlok = new Blok(config); const editor = await pending.isReady;`.",
      },
      {
        name: "isReady",
        type: "Promise<Blok>",
      ...
      },
      {
        ...
        description:
          "Drag-select (rubber-band) control. It is also reachable inside a tool as `api.rectangleSelection`.\n\n- `startSelection(pageX, pageY, shiftKey?)` begins a rubber-band from page coordinates.\n- `endSelection()` resets the drag state and hides the overlay.\n- `isRectActivated()` reports whether a rubber-band is currently active.\n- `clearSelection()` drops the active flag.\n- `cancelActiveSelection()` aborts a selection in progress (clear + end).\n\nAnother selection system, for example table cell selection, calls it when it takes priority.",
      },
    ],
  },
  {
    ...
    title: "Configuration",
    description:
      "The configuration object passed to the Blok constructor. It is formally split into two types.\n\n- `BlokMountOptions` holds the options that are fixed for the life of the instance (holder, tools, i18n, and so on).\n- `BlokState` holds the LIVE fields: `readOnly` (including `hideControls`), `hideToolbar`, `toolbarPosition`, `inlineToolbar`, and the editor callbacks `onChange`, `onSave`, `onEnter`, `onSubmit`, `onBeforeRender` and `onAfterRender`.\n\nEvery `BlokState` field maps to a documented runtime setter (`readOnly.set`, `toolbar.setHidden`, `toolbar.setPosition`, `tools.setInlineToolbar`, `handlers.set`). Changing one never requires recreating the editor. The React, Vue and Angular adapters react to these props and inputs in place.\n\nCallback PRESENCE is itself load-bearing: an `onSubmit` turns Enter into serialize-and-submit, and an `onSave` arms the change pipeline. That is why `handlers.set` also accepts `undefined`, to unset one.\n\n`BlokConfig = BlokMountOptions & BlokState`, so existing code compiles unchanged.",
...
const editor = new Blok(config);`,
    table: [
      ...
      },
      {
        ...
        default: "{}",
        description:
          "Available block and inline tools. Nothing is registered by default. The `{}` default leaves only Blok's internal tools (`stub`, `delete`, `copyLink`, `convertTo`), so a bare `new Blok({ holder })` cannot render even a paragraph.\n\nTwo ready-made bundles are exported from `@bloklabs/core/full`.\n\n- `defaultTools` holds paragraph, header and list, all with `inlineToolbar: true`.\n- `allTools` holds `defaultTools` plus quote, callout, code, toggle and every inline tool.\n\nSetting `toolbox: false` on a tool keeps it registered, so existing blocks still render and blocks.insert() still works. It removes the tool from every user-insertion path: the + / slash menu, the convert menu, and its keyboard shortcut. That is useful for permission gating.\n\nYou can flip it at runtime via `tools.update(name, { toolbox })`. The React adapter applies changes to the `tools` prop's `toolbox` values automatically. So a permission change never requires recreating the editor.",
      },
      {
        option: "tunes",
        type: "string[]",
      ...
      },
      {
        ...
        default: "false",
        description:
          "Opt-in: clicks on the host page below the editor append a block, with zero layout footprint. Pair it with `minHeight: 0` to remove the bottom zone entirely.\n\nOnly clicks landing on the empty background of an element that contains the editor count. Clicks on your own content rendered below are ignored. Propagation is never stopped, so host click handlers keep working alongside.",
      },
      {
        option: "inlineEmoji",
        type: "boolean",
        default: "true",
        description:
          "Inline emoji menu. Typing `:` followed by a name in a text block opens it, for example `:fire`. Picking an emoji replaces the typed `:query` with the character.\n\nThe colon opens the menu only at the start of a word. That means at the start of the block, or right after a space. `10:30`, `http://` and `Note: this` do not open it. At least one character must follow the colon.\n\nTyping a closing `:` can insert the emoji right away. That only happens when the query is an exact shortcode. `:fire:` gives 🔥 without the menu ever opening. Any other query is left as typed text.\n\nEscape closes the menu. It leaves the typed text unchanged. The page does not scroll while the menu is open. The menu's own grid still scrolls.\n\nThe menu has no search field, no random button, and no remove button. It also has no curated section. Those stay on the Callout tool's own emoji picker, which this option does not affect.\n\n`false` turns the menu off. An absent key keeps it on.",
      },
      {
        option: "find",
        type: "boolean | FindConfig",
        default: "true",
        description:
          "Find in page. Cmd/Ctrl+F inside the editor opens Blok's own find bar instead of the browser's. The bar opens at the top right of the window, where browsers put their own find bar, and stays above everything on the page. If you selected a short piece of text on one line, it fills the search field.\n\nPass an object to choose where the bar sits, for example `find: { placement: 'bottom-center', offset: { y: 24 } }`. `placement` is `top-end` (the default), `top-start`, `top-center`, `bottom-end`, `bottom-start` or `bottom-center`. `start` and `end` follow the editor's text direction.\n\n`offset` is the distance in pixels from those window edges. Use it to clear your own fixed UI, such as a header. The bar is fixed there: people using the editor cannot move it.\n\nMatching is loose by default.\n\n- Case is ignored. The `Aa` button (Alt+C) turns on match case.\n- Accents are ignored, so `cafe` finds `café`.\n- Curly and straight quotes match each other. So do the different dashes.\n- A run of spaces matches a single space.\n- The `ab` button (Alt+W) keeps only whole-word matches.\n\nA match can run across bold, italic and links. It never spans two blocks or two table cells. Search covers read-only content, code blocks, table cells and the children of collapsed toggles.\n\nEnter and Shift+Enter move to the next and previous match. Cmd/Ctrl+G and F3 do the same, and Shift makes them go back. A counter shows your place, such as `3 of 12`. A strip under the field marks where each match sits in the document. Click it to jump there.\n\nTyping never opens a collapsed toggle, but the matches inside it still count. Moving to one of them opens its toggle. That goes for the keys, the arrow buttons and the match map. In an editable editor this is a normal change, the same as clicking the toggle's arrow. It is saved and can be undone.\n\nEscape closes the bar and selects the current match, so typing replaces it. Press Cmd/Ctrl+F again while the bar has focus to use the browser's find for the whole page.\n\nOpen replace with the chevron button, with Cmd+Option+F on macOS, or with Ctrl+H elsewhere. In the replace field, Enter replaces the current match and moves to the next one. Cmd/Ctrl+Enter replaces every match as a single undo step. Replace is not available in read-only mode.\n\nWhile you type a replacement, the text shows the result before you replace. Each match is struck out, with the replacement right after it, and lines wrap the way they will. The preview is not saved, is not part of undo, and cannot be clicked.\n\nMatches are painted with the CSS Custom Highlight API. No markup is added to your content, so finding never changes the document. In a browser without that API the matches get no fill. The ring around the current match still shows, and stepping still works. The colors are theme tokens, listed in the Styles API section.\n\nCmd/Ctrl+F in your own page's inputs still opens the browser's find. With nothing focused on the page, it opens Blok's find. With several editors, it goes to the one used last. `false` leaves Cmd/Ctrl+F to the browser everywhere. An absent key keeps find on.",
      },
      {
        option: "defaultBlock",
        type: "string",
      ...
      },
      {
        ...
        default: "undefined",
        description:
          "Initial data to render. The loose wire shape is accepted: `null` values for block `data`, `id`, or `time` (common in backend DTOs) are normalized at the boundary. A whole-document `null` is also accepted and normalized to an empty document.\n\nSo you can pass nullable controlled state straight through, without a `value ?? { blocks: [] }` guard.",
      },
      {
        option: "dataModel",
      ...
      },
      {
        ...
        default: "undefined",
        description:
          "Fires when Enter is pressed in a block, before Blok splits it or creates a new one. Return true to mark it handled, and Blok suppresses its default block split or create (the native newline is still prevented).\n\nIt never fires for tools with enableLineBreaks, or while a popover or toolbar owns Enter, or for a soft-line-break Shift+Enter. The one exception is iOS: Safari reports Shift+Enter for a sentence-ending '. ', and Blok creates a block, so the hook fires there too.\n\nIt suits chat inputs (\"Enter sends\"). Pair it with the paragraph tool's preserveBlank config instead of subclassing Paragraph.\n\nLive field: install, replace or unset it at runtime via `handlers.set({ onEnter })`.",
      },
      {
        option: "onSubmit",
        type: "(data: OutputData, api: API) => void",
        default: "undefined",
        description:
          "Fires with the full serialized OutputData on the Enter that would otherwise create or split a block, the \"Enter sends\" gesture. Blok serializes the document and suppresses the default split, so you do not wire save() into onEnter by hand.\n\nIt inherits every onEnter escape. When both are set, an onEnter that returns true takes precedence and suppresses onSubmit.\n\nLive field: install, replace or unset it at runtime via `handlers.set({ onSubmit })`. Pass `undefined` to restore Blok's default Enter (split the block) without recreating the editor.",
      },
      {
        option: "onError",
        ...
        default: "undefined",
        description:
          "Fires when an editor operation fails that Blok would otherwise only log. Today the sole source is serialization. Both the debounced auto-save and an explicit save() route through it.\n\nA failed save() rejects with the underlying error and never resolves with undefined, so wrap explicit saves in try/catch. onError additionally surfaces failures of the debounced auto-save, which has no promise of its own.",
      },
      {
        ...
        default: "undefined",
        description:
          "Transforms the raw `text/html` clipboard payload before any Blok preprocessing or sanitization, so you no longer need a capture-phase paste interceptor.\n\n- Return the HTML to feed into the rest of the paste pipeline.\n- Return null to skip the HTML path and fall through to plain text.\n\nEverything below runs after your hook. Blok normalizes what other apps put on the clipboard.\n\nCopy an answer out of ChatGPT, Claude or Gemini, or a page out of Notion or Google Docs. It arrives as real blocks, not one flat paragraph. That means headings, lists, tables, quotes and code.\n\nChatGPT and Gemini get a dedicated pre-pass on top, because each hides meaning in markup the sanitizer would otherwise drop.\n\n- ChatGPT ships no MathML. So a formula's LaTeX is recovered from its source attribute and rebuilt as an equation.\n- Its code blocks are also de-duplicated. Each one renders as a nested editor.\n- Gemini's code language is read off the label it prints above the block.\n\nClaude has no pre-pass of its own: its answers are already semantic HTML, so they come through the standard HTML and markdown paths.",
      },
      {
        ...
        default: "undefined",
        description:
          "Transforms the blocks array just before it is rendered. It runs on the initial render, on every `blocks.render()` call, and on the repaints Blok performs itself. Those repaints are a runtime `i18n.update()` and the read-only fallback re-render.\n\nIt receives the raw saved blocks, before format analysis or hierarchical expansion, and returns the blocks to render. That way app-specific data migrations run inside Blok instead of ahead of it.\n\nIt must therefore be idempotent: those repaints feed it blocks it has already transformed. Live field: install, replace or unset it at runtime via `handlers.set({ onBeforeRender })`.",
      },
      {
        ...
        default: "undefined",
        description:
          "Fires after each render batch lands in the DOM: the initial render, every `blocks.render()`, and the repaints Blok performs itself.\n\nThose are a runtime `i18n.update()` locale or messages change, and a `readOnly.set()` toggle that falls back to a full re-render because a mounted tool does not support in-place read-only.\n\nUse it for post-render side effects such as scroll restoration or attaching observers. Keep those extra triggers in mind if you count renders. Distinct from onReady, which fires once when the editor first becomes ready.\n\nLive field: install, replace or unset it at runtime via `handlers.set({ onAfterRender })`.",
      },
      {
        option: "autofocus",
        type: "boolean",
      ...
      },
      {
        ...
        default: "'left'",
        description:
          "Which side of the content column the floating block controls (plus button and drag/settings handle) occupy. `'right'` moves both the controls and the gutter reserved for them to the editor's inline-end side. The start gutter collapses and an equal one opens at the end, so the text reclaims the space the controls used to occupy.\n\nThe values name the LTR-physical side and are applied through logical properties, so an RTL editor mirrors them. No effect while `hideToolbar` is on, or in chromeless read-only, because there are no controls to place.\n\nLive field: move them at runtime via `toolbar.setPosition(position)`.",
      },
      {
        option: "i18n",
        type: "I18nConfig",
        default: "undefined",
        description:
          "Internationalization config (locale plus message dictionary). Live field: switch language at runtime via `i18n.update({ locale, messages })`.\n\nThe editor relabels in place, so caret and undo history survive a language switch. `defaultLocale` is the exception and stays mount-only.\n\nCustom tool titles are localizable by registration name, for example a `fileLink` tool via `messages: { 'toolNames.fileLink': '…' }`, or via a `titleKey` in the tool's toolbox entry.",
      },
      {
        option: "uploader",
        type: "BlokUploader",
      ...
      },
      {
        ...
        default: "undefined",
        description:
          "Load the document on mount and save it as it changes, against your own endpoint. The Blok service stores no documents. It takes two callbacks rather than a URL, because the endpoint shape, its auth and the document id are yours.\n\nSaves never run in parallel, and only the newest pending document follows the one in flight. So a slow save finishing after a fast one cannot bring stale content back. Loading only happens when you passed no `data`, and setting `onSave` yourself wins.\n\n`load` may answer with a version alongside the document. Each `save` is told the version it is overwriting and may report the one it wrote. Blok only carries that version between the two calls, so your endpoint stays the only place a stale write is detected.",
      },
      {
      ...
      },
      {
        ...
        default: "undefined",
        description:
          "Fires with the RESOLVED theme ('light' or 'dark') whenever it changes. That covers the OS preference flipping while `theme` is 'auto', and `theme.set()` changing what the theme resolves to. It does not fire on initialization, and it does not fire when a `theme.set()` leaves the resolved theme unchanged.\n\nIt is a core config option, not an adapter-only prop. The framework adapters expose the same callback as the `onThemeChange` prop, the `theme-change` emit and the `themeChange` output.",
      },
      {
        option: "link",
      ...
      },
      {
        ...
        default: "undefined",
        description:
          "Identity of the current editor. Blok stamps `user.id` onto the `lastEditedBy` of every block this user edits, and without it `lastEditedBy` stays null.\n\nAdd `name` and the block settings footer reads \"Last edited by <name>\" with no callback to wire. That name is local only, so peers name you from `collaboration.user.name`. Set both to be named everywhere.\n\nFor other people's ids Blok asks `resolveUser`, and in a collaborative room it also learns names from the peers themselves.\n\nThe id is not private under collaboration: it goes into the shared document as `lastEditedBy`, and, only when `collaboration.user.name` is set too, into the presence state next to that name. That is how peers name each other. Pass an opaque per-document token instead of an internal account key if that matters.",
      },
      {
        option: "resolveUser",
        ...
        default: "undefined",
        description:
          "Resolves the `lastEditedBy` user id Blok shows in the block settings footer. It may return synchronously or asynchronously. Return null for an unknown user, and Blok falls back to the name that user published to the collaborative room, or to the date alone.\n\nBlok asks at most once per id: it keeps both an answer and an \"I don't know this one\" for the editor's lifetime. A callback that throws or rejects is absorbed rather than breaking the menu, and because that is not an answer, the id is asked about again later.\n\nWiring this callback also means an unauthenticated name from the room is never shown while the lookup is still running.\n\nThe id arrives trimmed and stripped of NUL. One longer than 128 characters is refused rather than passed on.",
      },
      {
...
const block = editor.blocks.insert('column_list', undefined, undefined, index, undefined, undefined, undefined, undefined, 'user');`,
        params: [
          ...
          },
          {
            ...
            default: "{}",
            description:
              "Ignored. It is accepted only to keep the positional signature stable. The editor binds it to `_config` and never reads it. The block-manager insert options carry no `config` field, so the created block still uses the editor-level tool config from `config.tools`.\n\nPass `undefined`.",
          },
          {
            name: "index",
            type: "number",
          ...
          },
          {
            ...
            default: "'api'",
            description:
              "Why the block is being created. It is handed to the tool constructor as `origin`. That is how a container tool tells a genuine creation (seed default children) from a re-materialisation, such as a document load, an undo/redo replay or a paste (never seed).\n\n- Pass `'user'` when the insert comes from your own insertion UI: a custom toolbar, slash menu or keyboard shortcut.\n  - Then Blok's own containers (`column_list`, `column`) and your own behave the same as they do for the built-in menu.\n- Leave it out for programmatic inserts and refetches.\n  - The `'api'` default is still a creation, but it is never mistaken for a user gesture.",
          },
        ],
        errors: [
          {
...
// Returns: { text: '' } or appropriate empty state for the tool`,
      ...
      },
      {
        ...
        description:
          "Updates a block's data and/or tunes. The data update is applied IN PLACE when the block's tool implements `setData(newData)` on its prototype. Every React/Vue/Angular block does, as do the built-in header, list, code, toggle and table tools.\n\nIn place means the same block instance, the same DOM holder and the same mounted component keep living. So ephemeral tool state, adopted child blocks and the caret all survive. That is what makes `update()` safe to call on every keystroke, for example when renaming a card while the user types.\n\nThree cases fall back to recomposing the block:\n\n- tools without `setData`\n- a tool whose `setData` returns `false` (it needs a different DOM shape)\n- any call that passes `tunes`\n\nRecomposing means a fresh tool instance replaces the old one, which is destroyed.",
...
// A single Cmd+Z removes both blocks`,
      },
      {
        ...
        returnType: "void",
        description:
          "Opens an undo group that stays open across async boundaries. Every block operation until `endTransaction()` lands in one undo entry.\n\nUse it for pointer gestures that mutate the document continuously, such as dragging a table's corner to add rows. `transact()` cannot help there, because it only wraps a synchronous function. Pair every call with `endTransaction()`.",
...
block?.setParent(null);`,
      },
      {
        ...
        returnType: "BlockAPI",
        description:
          "Insert a child block under THIS block atomically. Creation and parent assignment land in a single undo entry (it delegates to `blocks.insertInsideParent`).\n\n- `position` is a `BlockChildPosition`: 'start' | 'end' | { before: childId } | { after: childId }.\n  - It defaults to 'end', which appends past the whole subtree.\n- `toolName` picks the child's block tool and defaults to `config.defaultBlock`, so a TYPED child is one operation instead of insert-then-reparent.\n  - A tool that is restricted inside table cells is demoted to the default block when the new child would land inside one.\n- `childData` defaults to `{ text: '' }` for the default block, and to `{}` when `toolName` names a different tool.\n- `options` uses the same `{ focus, caret, id, tunes, replace }` vocabulary as the framework adapters' rich `insert` spec.\n\nSo a container tool never has to place the caret by hand, or follow up with an `update` to apply tunes.",
        example: `const block = editor.blocks.getById('toggle-123');
...
block?.insertChild({ text: 'Now a heading', level: 3 }, { before: 'child-id' }, 'header', { replace: true });`,
        params: [
          ...
          },
          {
            ...
            default: "{}",
            description:
              "focus makes the new child the current block. caret places the caret inside it ({ position?, offset? }), and applies only when a child is actually created.\n\nid sets an explicit id. An id that already exists works as insert-if-absent: that child is returned and nothing is created.\n\ntunes is block tune data applied at creation. replace overwrites the child named by an object position instead of inserting beside it. With 'start' or 'end' there is nothing to overwrite, so the call throws.",
          },
        ],
      },
      {
    ...
    methods: [
      {
        ...
        returnType: "boolean",
        description:
          "Whether every text node in the range is inside a wrapper matching the spec. Whitespace-only text nodes are ignored. At a collapsed caret, the caret's ancestors are checked.\n\nselection.findParentTag checks only the selection's boundary nodes and their ancestors. This method is stricter: a selection that only partially carries the mark reports false.",
        params: [
          {
            name: "spec",
            type: "MarkSpec",
...
const current = snapshot?.style['color']; // e.g. 'rgb(37, 99, 235)'`,
      },
      {
        ...
        description:
          "Wrap the range in the mark, or update matching wrappers in place.\n\n- Splits partially-covered same-family wrappers at the range boundaries.\n- Extends the range over trailing whitespace that browsers exclude from double-click selections.\n- Leaves the new contents selected.\n- Returns the created or updated wrapper elements.\n\nA collapsed range (a bare caret with nothing selected) is a no-op: apply returns an empty array without touching the DOM or the selection.",
        params: [
          {
            name: "spec",
            type: "MarkSpec",
...
editor.marks.apply(colorMark, { color: '#2563eb' });`,
      },
      {
        ...
        returnType: "HTMLElement[]",
        description:
          "Removes the spec's declared properties and classes from wrappers in the range, and unwraps wrappers left bare.\n\n- For a non-collapsed range, partially-covered wrappers are split so text outside the range keeps its formatting.\n- A collapsed caret instead targets its enclosing wrapper whole: no split happens, and the spec's properties are stripped from the entire wrapper.\n\nEither way, the selection is restored. Returns the wrappers that survived because they still carry other properties.",
        params: [
          {
            name: "spec",
            type: "MarkSpec",
...
// with only the colour stripped`,
      },
      {
        ...
        returnType: "boolean",
        description:
          "Calls remove when the range already carries the mark, and apply otherwise. Returns the resulting state: true when the mark is now applied.\n\nAt a collapsed caret with no existing mark it returns true without applying anything, because apply is a no-op on a collapsed range. Call toggle on a non-collapsed range, or treat the return value as the intended state rather than a confirmation.",
        params: [
          {
            name: "spec",
            type: "MarkSpec",
...
const nowApplied = editor.marks.toggle(highlight);`,
  ...
  },
  {
    ...
    title: "Styles API",
    description:
      "Access CSS class names for styling custom tools and UI elements. You can also customize the editor's layout and chrome through public CSS custom properties.\n\nThe main way to override theme tokens is `style.tokens` in the Blok constructor config. Pass `--blok-*` keys and values. Blok injects a per-instance stylesheet that reaches the editor and, automatically, UI portaled to `document.body` (popovers, tooltips, top-layer elements).\n\nInvalid keys are skipped with a warning. The stylesheet is removed on destroy.\n\nInjected `style.tokens` values are static per application. They apply identically in light and dark themes and across read-only state. So state-dependent tokens like the editor gutter belong in CSS instead. `style.tokens` ignores `--blok-editor-gutter-*` keys with a warning.\n\nThey are not frozen at construction, though. `editor.tokens.set(tokens)` rewrites the injected stylesheet at runtime, which is what a host light/dark toggle needs. Without it, flipping a token meant recreating the editor or hand-writing a global stylesheet that targets the portal scopes yourself.\n\n`set()` takes the complete token set: it replaces, it does not merge, mirroring `style.tokens`. Tokens you leave out of the new palette stop applying, and `{}` removes the stylesheet. `editor.tokens.get()` returns what is currently applied.\n\nThe API is available synchronously right after construction. Calls made before `isReady` are buffered and replayed. The React, Vue and Angular adapters drive it reactively: pass `style={{ tokens }}` (React/Vue) or `[styleTokens]` (Angular), and changes sync in place without recreating the editor.\n\nThere is a CSS-only alternative. Blok declares its own palette at zero specificity via `:where()`. So one plain selector like `[data-blok-interface] { --blok-popover-bg: … }` wins whatever the stylesheet order. But popovers portal to `document.body`, so that global stylesheet must also target `[data-blok-popover], [data-blok-top-layer]` to reach them.\n\n`--blok-content-max-width` stays authoritative in both width modes. `width='full'` only swaps its fallback to `none`.\n\nIn edit mode Blok automatically reserves 56px of gutter for the floating +/⠿ block controls. The wrapper carries `data-blok-readonly` while read-only is active. Plain read-only KEEPS the gutter.\n\nThe block-hover copy-link control lives there. And `readOnly.set()` flips modes in place, so collapsing the gutter would shift the document sideways on every toggle.\n\nThe gutter collapses to 0 automatically only where it is genuinely dead space.\n\n- Chromeless read-only: `readOnly: { hideControls: true }`, where the wrapper carries `data-blok-controls-hidden`.\n- `hideToolbar: true` in the constructor config. There the hover toolbar never opens, the wrapper carries `data-blok-toolbar-hidden`, and no gutter space is reserved.\n\n`--blok-editor-gutter-start` is an override hook, not a required incantation. Set it to any value to change the default, including `0px` to remove the gutter.\n\nThe gutter override contract is guaranteed, not incidental. Blok declares the gutter default and both state collapses at zero specificity via `:where()`. A unit contract test enforces that. So a host declaration of the gutter tokens at any positive specificity always wins the cascade.\n\nDeclare them on the wrapper element itself, for example `[data-blok-interface] { --blok-editor-gutter-start: 16px }`, not only on an ancestor. The controls-hidden and toolbar-hidden collapses re-declare the tokens on the wrapper, and custom properties resolve from the nearest declaration. So an ancestor-level value loses to the collapse, while a wrapper-level one survives it.\n\nThe content column's horizontal position is configurable at the API level too. Use `style.contentAlign?: 'left' | 'center' | 'right'` (default `'left'`) in the Blok constructor config.\n\nBlok also repaints native text selection inside the editor with `--blok-selection-inline`. Override that token to recolor it. Or pass `style.nativeSelection: true` (default `false`) to opt out entirely. Blok then falls back to the selection colors defined by the browser or the host.\n\nA token override cannot express CSS-wide keywords like `revert`, so reverting needs this flag. With the flag on, the wrapper carries `data-blok-native-selection` and Blok's `::selection` rules skip the editor. The fake-background highlight, shown while a menu input holds focus, then follows the UA `Highlight` color. Popovers keep Blok's selection color.\n\nBackground surfaces are public tokens too.\n\n- Most hover and light UI surfaces follow `--blok-bg-light`.\n- Media empty-state cards use `--blok-bg-secondary`, bordered by `--blok-border-secondary`.\n- The image and file loading skeletons and the upload placeholders use `--blok-bg-tertiary`.\n  - That token defaults to `--blok-bg-light`, so it tracks the theme.\n\nTo recolor the skeleton surface, override `--blok-bg-tertiary` directly. Do not overload `--blok-bg-light` and drag every other surface along with it.\n\nLike all palette-backed color tokens, the surface tokens are re-declared by Blok on the editor wrapper at zero specificity. So apply overrides through `style.tokens`, `editor.tokens.set()`, or a CSS selector matching the wrapper (`[data-blok-interface]`) itself. A custom-property declaration on an ancestor container is shadowed by the wrapper's own declaration and silently does nothing.\n\nFind in page has its own tokens.\n\n- `--blok-find-match-bg` fills every match, and `--blok-find-match-active-bg` fills the current one. They are painted under the text, so keep them translucent.\n- `--blok-find-lens-ring` draws the ring around the current match.\n- `--blok-find-tick` and `--blok-find-tick-active` color the marks on the match map, and `--blok-find-track` the line under them.\n- `--blok-find-empty-bg` and `--blok-find-empty-border` color the search field when nothing matches.\n- `--blok-find-bar-width` (default 432px) and `--blok-find-control-size` (default 28px) size the bar.\n\nBlok declares these on the editor wrapper, like the palette. Set them through `style.tokens` or a rule on `[data-blok-interface]` itself.\n\nLayout hooks such as `--blok-content-max-width` are read with fallbacks and never declared by Blok. The same is true for the list, heading, embed, block-padding and placeholder-color tokens. That is why those DO inherit from any ancestor. The gutter tokens and `--blok-search-input-placeholder` are wrapper-declared like the palette, so they too need a wrapper-level rule.\n\nNote that the injected token stylesheets target Blok's scope attributes globally. With several editor instances on one page, each instance's `style.tokens` / `tokens.set()` stylesheet applies to ALL Blok UI on the page. It does not stop at its own instance. Each sheet is removed when its own instance is destroyed.\n\nWhere sets conflict between instances, the stylesheet order in `<head>` decides, not application recency. So give every instance one shared set instead of relying on conflict order. Scope per-instance differences with a CSS rule on each editor's own wrapper instead. Body-mounted popover UI always follows the page-wide sheets.\n\nThe sheets are injected at the start of `<head>`. So a host stylesheet rule of equal specificity, a plain `[data-blok-interface] { … }`, still beats `style.tokens` for the tokens it declares.\n\nBlock rhythm is public too. `--blok-block-padding-top`, `--blok-block-padding-bottom` and `--blok-block-padding-inline` drive the padding of every block tool wrapper: paragraph, heading, list, toggle and quote. Each tool keeps its historical value as the fallback: 7px/7px/2px for most blocks, and 0.2em vertical for quotes.\n\nSo one override retunes all blocks at once. That is exactly what a read-only host needs for tight inline-style rendering. Previously that was only possible by overriding `[data-blok-tool]` internals.\n\nThe callout panel is the deliberate exception. Its card inset is `--blok-callout-padding-block` (default 5px), NOT the rhythm tokens. So tightening rhythm cannot collapse the callout card onto its text.\n\nThe emoji still stays on the first text line, because its button follows `--blok-block-padding-top` together with the child text. Note that non-default padding slightly shifts derived geometry, such as the toggle-heading arrow offset, which follows `--blok-block-padding-top`.\n\nColumn layout is public in the same way. A columns row is `[data-blok-columns]`, and each column holder is one of its direct `[data-blok-element]` children. A read-only row also carries `data-blok-columns-static-gutter`.\n\nPublished rows take their gutter from the container, not from the `[data-blok-column-resizer]` separators. Those separators only exist while editing.\n\n- `--blok-column-gutter` sets the gap (default `min(2rem, 4vw)`).\n- `--blok-column-min-width` sets how far a column may be squeezed.\n  - The default is `0`, so a column can be dragged all the way to collapse.\n\nBOTH the layout and the resizer drag honor that floor, because the drag reads the resolved value back at pointer-down. So raising it stops the handle at the floor instead of persisting a width the layout refuses to render.\n\nBlock nesting is public the same way. A block nested under another (Tab at root level) is indented by `--blok-block-indent-step` per level (default `24px`). It is real CSS, not an inline style, so a plain host rule retunes or removes it with no `!important`.\n\nBlok zeroes the step inside every `[data-blok-nested-blocks]` child slot. That is the marker every container tool renders for its children, first-party and third-party alike. So blocks a container already positions are never pushed sideways by their depth on top of that. A container that DOES want the indent declares the step back on its own slot.\n\nThe reset rides on inheritance rather than on a JS check. That is deliberate: it also holds for a slot created after the child was inserted. A framework adapter's portal does exactly that.\n\nText size is public per block AND per scenario through `style.fontSize`. That is the supported alternative to targeting Blok's internal class names. Every key writes one public token.\n\n- `fontSize.paragraph` → `--blok-paragraph-font-size`.\n- `fontSize.heading[1]` → `--blok-heading-1-font-size`.\n- `fontSize.list.checklist` → `--blok-checklist-font-size`.\n- The same holds for both quote variants, callout, code, toggle and the two table densities (`compact` / `comfortable`).\n- It also holds for every media caption (image, video, audio, file, embed) and the three bookmark parts (title, description, link).\n\nHeadings reuse the pre-existing heading tokens rather than minting parallel ones. Omitted keys keep Blok's built-in size, so an editor renders exactly as before everywhere it does not opt in.\n\nPer-tool size settings still outrank it. A paragraph tool configured with `styles.size` writes that size as an inline style on the block. So does a list with `itemSize`. No token can override an inline style.\n\nSo scenarios you want to drive from `style.fontSize` must not also carry a per-tool size. Values may be absolute or relative (`px`, `rem`, `em`, `%`).\n\nEvery ornament sitting beside sized text derives its own metrics from the same token. That covers a list bullet, checkbox, callout emoji or toggle arrow. So each one stays optically aligned at any scale, with no extra CSS.\n\nThese tokens are read with fallbacks. Blok never declares them on its own. So an editor that does NOT configure `style.fontSize` also accepts them from a plain CSS rule on any ancestor. It accepts them from `style.tokens` / `editor.tokens.set()` too.\n\nThe token NAMES ship as a constant. `import { BLOK_FONT_SIZE_TOKENS } from '@dodopizza/blok'` gives you a map shaped exactly like the config (`BLOK_FONT_SIZE_TOKENS.paragraph`, `BLOK_FONT_SIZE_TOKENS.heading[1]`, `BLOK_FONT_SIZE_TOKENS.bookmark.link`…). A host that scopes typography from CSS then never hand-copies the strings. And a rename becomes a compile error rather than a silent no-op.\n\nThe channels compose. `style.fontSize` is the construction-time value, and `editor.tokens.set({ [BLOK_FONT_SIZE_TOKENS.paragraph]: '18px' })` overrides it at runtime. The theme-token sheet is injected directly after the fontSize sheet at equal specificity, so it wins.\n\nThat is the channel for a size that must change after mount, such as a density, zoom or accessibility toggle. `style.fontSize` itself is read once at construction.\n\nUnlike `style.tokens`, the injected fontSize sheet is scoped to its own editor. The wrapper carries `data-blok-instance`, and the sheet's editor selector is keyed to it. So a second editor on the page keeps Blok's built-in sizes, or its own config. It does not inherit the first one's.\n\nThe one part that stays page-wide is body-mounted UI. Popovers and tooltips render outside every editor's subtree, so those rules follow `<head>` order when instances disagree.\n\nOne nesting rule is worth knowing. A callout renders its body text as a child paragraph block. So callout text follows `fontSize.callout`. It falls back to `fontSize.paragraph` when that key is unset.\n\nSetting only `paragraph` resizes callout bodies along with body text. To make the two differ, set `fontSize.callout` explicitly.\n\nFinally, the view renderer (`@bloklabs/core/view`) emits semantic HTML, and its stylesheet carries only the class-based scenarios. Paragraph, headings, list, checklist, both quote sizes, callout, code and toggle respond in view output. The caption, table-cell and bookmark sizes are editor-only.",
...
new Blok({
  style: {
    tokens: {
      ...
      // Find-in-page match fills are painted UNDER the text, so keep them
      // translucent or the glyphs lose contrast.
      '--blok-find-match-bg': 'rgba(255, 212, 0, 0.32)',
      '--blok-find-match-active-bg': 'rgba(255, 138, 0, 0.5)',
    },
  },
});
...
editor.toolbar.toggleToolbox(true);`,
      },
      {
        ...
        returnType: "void",
        description:
          "Runtime setter for `config.hideToolbar`. It hides or shows the hover toolbar (plus button and drag handle). It also collapses or restores the editor gutter reserved for it.\n\nThe wrapper's `data-blok-toolbar-hidden` attribute is kept in sync, so no dead space is left behind. The keyboard \"/\" menu keeps working while the toolbar is hidden.",
        params: [
          {
            name: "hidden",
            type: "boolean",
...
editor.toolbar.setHidden(false);`,
      },
      {
        ...
        returnType: "void",
        description:
          "Runtime setter for `config.toolbarPosition`. It moves the floating block controls between the editor's inline-start and inline-end gutters. The wrapper's `data-blok-toolbar-position` attribute is kept in sync. That attribute drives both the gutter swap and the side the controls dock to.\n\nThe block-settings menu mirrors with them, so it never opens over the block it belongs to. An open toolbar is re-laid out in place rather than closed.",
        params: [
          {
            name: "position",
            type: "'left' | 'right'",
...
editor.toolbar.setPosition('left');`,
  ...
  },
  {
    ...
    title: "Notifier API",
    description:
      "Display notification messages to users. Rendering is pluggable. Pass `notifier: (options) => …` in the constructor config and Blok calls your handler instead of rendering anything.\n\nThe built-in toast is skipped entirely, including its i18n `okText`/`cancelText` defaults. Any error your handler throws propagates to the `show()` call site.\n\n`notifierPosition` places the built-in container: 'bottom-left' | 'bottom-right' | 'bottom-center' | 'top-left' | 'top-right' | 'top-center' (default 'bottom-center').",
    methods: [
      {
  ...
  okHandler: (value) => console.log('Entered:', value)
        ...
        params: [
          ...
          },
          {
            ...
            required: false,
            default: "undefined",
            description: "Marks the notification's semantic kind. With `'error'` the message's screen-reader live region is `assertive`. Otherwise it is `polite`. The value is also stamped onto `data-blok-testid` as `notification-success` / `notification-error`.\n\nThe built-in toast looks the same either way. There is no success/error coloring.",
          },
          {
            name: "options.time",
            type: "number",
    ...
    properties: [
      {
        ...
        type: "'plaintext'",
        description:
          "A field-level sanitizer rule a tool declares in `static get sanitize()`. It is part of the `SanitizerRule` union exported from the package root.\n\nSanitization is an HTML parse: it entity-encodes bare `<`/`&` and drops text shaped like a stray end tag. For a field holding literal source text, such as a code block's `code`, that is irrecoverable corruption.\n\nA field marked `'plaintext'` skips tag sanitization, the URL-scheme pass and the editor-level global sanitizer, and round-trips byte-identical. It is declared as a plain string literal rather than a Symbol, so tool sanitize configs survive JSON and `structuredClone`.",
      },
    ],
  },
  {
...
editor.placeholder.set(false);`,
  ...
  },
  {
    ...
    title: "ReadOnly API",
    description:
      "Control the read-only state of the editor. Toggling happens in place as long as every registered block tool implements `setReadOnly(state)` on its prototype. Every bundled tool does.\n\nThe same editor instance then flips modes and keeps caret position, undo history and scroll. So an edit/view toggle is `readOnly.set(!isEditing)` on ONE instance, instead of destroying one editor and constructing another.\n\nThe check is all-or-nothing. Install a single block tool without `setReadOnly` and every toggle falls back to a save, clear and re-render cycle. That cycle recreates all block instances and does not restore the caret. Scroll is still restored, and the undo history is deliberately left untouched.",
    ...
    methods: [
      {
        ...
        returnType: "Promise<boolean>",
        description:
          "Set read-only mode to the specified boolean state. The toggle happens in place, with no destroy or recreate step: block instances, caret position, undo history and scroll are all preserved.\n\nThat holds only if every registered block tool implements `setReadOnly(state)` on its prototype. The check is all-or-nothing.\n\nOne tool without it sends EVERY toggle down the fallback path of save, clear and re-render (every bundled tool has one, a third-party tool may not). That path recreates all block instances and does not restore the caret, though scroll and undo history still survive.\n\nPass `{ hideControls: true }` to also hide the hover toolbar, block settings and inline toolbar while read-only is active. The option writes the object form of `config.readOnly`, so the live state reflects it. Returns the new state.",
        note:
        ...
        params: [
          ...
          },
          {
            ...
            description:
              "Hide all editor controls (hover toolbar, block settings popover, inline toolbar) while read-only is active.\n\nThe value is sticky. `set()` writes `config.readOnly` only when you pass an actual boolean, so omitting the option keeps whatever `hideControls` is already in effect, from the constructor config or from an earlier `set()` call.\n\nPass `{ hideControls: false }` explicitly to bring the controls back. `false` is the effective value only when `config.readOnly` was never given in object form.",
          },
        ],
...
await editor.readOnly.toggle(false);`,
    ...
    properties: [
      ...
      },
      {
        ...
        type: "true",
        description:
          "A build-level marker, hardcoded to `true`. It says this build of Blok has the in place toggle path, instead of always recreating the editor.\n\nIt is not a capability probe. It does not report whether the currently installed tool set qualifies for that path. That path needs every block tool to implement `setReadOnly`, and it is not exposed anywhere.\n\nUse the marker only to detect a Blok build old enough to predate in-place toggling.",
      },
    ],
  },
  {
    ...
    title: "I18n API",
    description:
      "Internationalization support for translating UI strings. The runtime `i18n.update()` mutator switches language in place.\n\nThe locale catalogue itself ships as a separate published entry point, `@bloklabs/core/locales`. Only English is bundled, and the other 68 locales load on demand.\n\nUse `normalizeLocale()` as the pre-flight check for a locale you did not hard-code. `i18n.update({ locale })` with an unsupported tag keeps the current locale and warns on the console instead of throwing.",
    methods: [
      {
...
console.log(locale); // 'en'`,
      },
      {
        ...
        returnType: "'ltr' | 'rtl'",
        description:
          "Get the text direction currently in effect. It is derived from the active locale, unless an explicit `direction` override was set. This lives on the editor instance only.\n\nThe `api.i18n` handed to tools carries just `t`, `has`, `getEnglishTranslation` and `getLocale`. So a tool must take the direction from the host, either its own config or the `i18n:changed` event, rather than calling this.",
  ...
  // mirror your own chrome next to the editor
      ...
      },
      {
        ...
        returnType: "Promise<void>",
        description:
          "Switch language at runtime. `config.i18n` is otherwise read once during boot, so a host with a language switcher had to recreate the editor to relabel it. That threw away caret, focus, selection and undo history. `update()` relabels in place instead: no recreation, nothing lost.\n\n- `locale` accepts any supported code, or `'auto'` to re-run browser detection.\n- `messages` merges host overrides over the locale dictionary. It is re-applied automatically after every later locale change, so a bare locale flip never silently drops your custom strings.\n- `direction` overrides the direction implied by the locale, which you normally do not need.\n\nCalls are serialized internally, so lazily-loaded locale chunks cannot land out of order. The last call wins.\n\nThe scope is everything.\n\n- UI built on demand (block settings, the convert menu, notifications, screen-reader announcements) picks up the new locale the next time it opens.\n- UI stamped up front (toolbar and plus-button labels, tooltips, the toolbox list) is relabelled immediately.\n- Block content is repainted from your data: placeholders, media-toolbar labels, cell controls, anything a tool resolved while rendering. That includes tools that know nothing about locale changes.\n\nThe repaint is invisible to you. `onChange` does not fire, scroll is kept, and the caret returns to the block that had it.\n\nFires the `i18n:changed` event with `{ locale, direction }`. It is available synchronously after construction, and a call made before `isReady` is applied once the editor has booted.\n\n`update()` and `getDirection()` are exposed on the editor instance only, not on the `api.i18n` handed to tools, which carries just `t`, `has`, `getEnglishTranslation` and `getLocale`. So a third-party tool cannot flip the host's locale.\n\nThe React, Vue and Angular adapters drive it reactively: change the `i18n` prop or input and the editor follows in place.\n\nNote that `defaultLocale` is not accepted. It only decides the fallback while resolving the initial locale.",
...
editor.events.on('i18n:changed', ({ locale, direction }) => {
  document.documentElement.dir = direction;
      ...
      },
      {
        ...
        description:
          "From `@bloklabs/core/locales`. Normalizes an arbitrary BCP-47 language tag to a supported Blok locale, and returns `null` when the tag is unsupported. The tag can be region-tagged (`'en-US'`), script-tagged (`'zh-Hant'`) or aliased (`'nb'` to `'no'`, `'ckb'` to `'ku'`).\n\nThe same normalizer runs on browser detection and on explicit `config.i18n.locale` or `i18n.update({ locale })`. So a `null` here is exactly the tag that `update()` would refuse: it keeps the current locale and warns on the console rather than throwing.",
    ...
    methods: [
      {
        ...
        description:
          "Get all available block tool adapters. Each adapter exposes `name` plus tool metadata, including `assetKind`.\n\n- On media tools that store an uploaded asset URL at `data.url`, `assetKind` is `'image' | 'video' | 'audio' | 'file'`.\n- On every other tool it is `undefined`.\n\nUse it to find the media-bearing tools at runtime, instead of hardcoding each tool's data shape. You can then check a saved document's `data.url`s against your CDN, for example to clean up orphaned uploads.",
...
const referenced = (await editor.save()).blocks
  ...
  .map(b => b.data.url);`,
      ...
      },
      {
        ...
        returnType: "void",
        description:
          "Shallow-merges new configuration into an installed tool at runtime, with no editor recreation. A `toolbox` key is treated as the tool-level setting, the same as `toolbox` in the `tools` map.\n\n- Pass `toolbox: false` to hide the tool from every insertion surface; existing blocks keep rendering.\n- Pass a toolbox object to show it again.\n\nThis gates permissions without rebuilding the editor. Under the React adapter it is automatic: change the `toolbox` value in the `tools` prop and `useBlok`/`BlokEditor` applies it in place.",
...
editor.tools.update('goodsList', { toolbox: { title: 'Goods List' } });`,
      },
      {
        ...
        returnType: "void",
        description:
          "Runtime setter for the global `inlineToolbar` config. It re-assigns inline tools for every block tool and recomposes the memoized sanitize configs.\n\n- Paste-time sanitization follows the new set immediately.\n- The inline toolbar reflects it on the next selection.\n- Tool-scoped `inlineToolbar` settings, both arrays and opt-outs, stay authoritative.\n\nPass `true` for all inline tools, `false` for none, or a list of inline tool names. The list selects which tools appear, while their left-to-right order stays fixed.\n\nIf you render saved content through @bloklabs/core/view, note that a viewSchema is composed from the inlineToolbar value it was defined with. After a runtime setInlineToolbar involving custom inline tools, recompose it with defineBlokSchema before calling blocksToHtml.",
        params: [
          {
            name: "config",
            type: "boolean | string[]",
...
editor.tools.setInlineToolbar(true);`,
  ...
  editor.tools.update('image', { uploader: { uploadByFile } });
      ...
      },
      {
        ...
        description:
          "A registration helper exported from `@bloklabs/core/tools`. It is not a member of the `tools` namespace.\n\nThe plain `tools` map types every entry with a bare `ToolSettings` whose `Config` falls back to `Record<string, unknown>`. So a misspelled config key (`defaultLevle` for `defaultLevel`) compiles silently. `defineTool` recovers the tool's real config type from its constructor and applies it to `settings.config`, turning those typos into compile errors.\n\nThe type-checking happens on the `settings` argument. The RETURN type stays the erased `ExternalToolSettings`, so the result drops straight into the `tools` map.\n\n`ExtractToolConfig<TClass>` is the type that does the recovery. It is exported alongside `defineTool`, and falls back to `Record<string, unknown>` for tool classes whose constructor declares no concrete config.",
  ...
  tools: {
    ...
    // \`defaultLevle: 2\` here would now be a compile error
      ...
      },
      {
        ...
        returnType: "void",
        description:
          "The child-holder reconciler for container blocks, exported from `@bloklabs/core/tools`. Call it from your tool's `rendered()` hook. The built-in toggle, callout and column tools use it, and the React/Vue/Angular block adapters run it on every commit.\n\nIt is idempotent and cheap, so run it on every render. Per child it behaves like this.\n\n- It leaves a holder already inside `container` alone.\n- It RECLAIMS a holder stranded in a nested container that ENCLOSES `container`, inserting it at its model position rather than appending it last.\n- It leaves holders sitting in any OTHER nested container alone, so two containers can never steal each other's blocks.\n- It mounts everything else at its model position.\n\nThe reclaim is what makes a container survive the insert ordering. Core anchors a newly inserted first child as the container block's DOM sibling, so without the reclaim a child of a nested container renders one level out. That is permanent when your container's child slot had not been created yet at insert time, which is what happens when a framework portal commits a render after core inserts.\n\nMark `container` with `data-blok-nested-blocks` so the rest of the editor recognises it as a container.",
        params: [
          {
            name: "container",
            type: "HTMLElement",
  ...
  rendered() {
    mountChildBlocks(this.slot, this.api.blocks.getChildren(this.blockId));
      ...
      },
      {
        ...
        description:
          "The create-vs-restore signal on the tool contract. Blok hands it to every block tool's constructor alongside `data`, `block` and `readOnly`.\n\nA container tool that seeds default children, such as a two-column row or a card that starts with a heading, may only do that once, at creation. Every other time the tool is constructed, the document already says what its children are.\n\nDuring a restore those children commonly land a tick AFTER `rendered()` runs, so an empty `api.blocks.getChildren()` there is only transient. Seeding on that read fabricates phantom children beside the real ones.\n\nCREATION values, where you may seed:\n\n- `'user'` (a direct editing gesture: Enter, the plus button, the slash menu, block settings, a markdown shortcut).\n- `'api'` (a programmatic `blocks.insert` / `insertMany` / `insertInsideParent`).\n- `'convert'` (a turn-into).\n\nRESTORE values, where you never seed:\n\n- `'load'` (a document render).\n- `'replay'` (an undo/redo replay or a remote collaborative update).\n- `'paste'` (pasted content that brings its own children).\n- `'probe'` (the OFF-TREE instance `blocks.composeBlockData()` builds to read a tool's default data).\n\nThe probe instance is never inserted, yet it still runs `render()` and `rendered()`, so it must not touch the block tree at all.\n\nBlok always supplies the value. Treat an absent value as `'api'`, and write the check as an allow-list of creation values, so a future origin fails closed. Pair it with `blocks.insert(..., origin)` if you drive insertion from your own UI.\n\nOn the React/Vue/Angular adapters you rarely read it by hand. The block spec's `onCreated` hook already encodes this allow-list. It fires after the adapter's first commit, the tick at which the block's DOM and its adopted child holders actually exist.",
  ...
  constructor({ data, origin, replaySource }) {
    ...
    this.refetch = origin === 'replay' && replaySource !== 'remote' && data.title === undefined;
      ...
      },
      {
        ...
        returnType: "boolean",
        description:
          "A static on your tool CLASS that decides where Enter goes on the container's empty LAST child. By default Blok reads that empty trailing line as the author's way out.\n\n- With siblings present the line is outdented to the container's own parent.\n- When it is the sole child, Blok inserts a fresh block after the whole container.\n\nThat is Notion's callout behaviour. A layout container whose children ARE its content (a column, a card, a `steps` block) wants the opposite. Such a tool needs the declaration: without it, the escape strands the new line beside the container.\n\nSet it to `true` and the new line stays inside. That is the same rule the built-in `column`, `column_list` and `toggle` follow. It cannot be inferred from the DOM: a callout renders the very same `data-blok-nested-blocks` slot as a column and deliberately keeps the escape, so this is per-tool policy.\n\nCore reads it for the symmetric \"remove one indent level\" gesture too (Enter/Backspace on a block nested under a PLAIN parent). A declaring tool is treated as a container there as well, and its children never stepwise-outdent out of it.\n\nOn the React/Vue/Angular adapters, declare it in the block spec's `statics` bag like any other class static.",
...
export const StepsTool = createReactBlock({
  ...
  component: StepsCard,
      ...
      },
      {
        ...
        description:
          "A static on your tool CLASS declaring which block tools may be DIRECT children of its block, and core enforces it everywhere for you.\n\n- On INSERT a disallowed tool is demoted, never refused, because Enter must always produce a block.\n  - The target is the first entry of `allow`, so `allow: ['segment-item']` makes \"Enter at the end of a segment\" produce another segment instead of a stray paragraph.\n- On MOVE a drag or keyboard reorder that would carry a disallowed block across the container boundary is refused.\n- In the TOOLBOX the disallowed tools are hidden while the caret sits in a child.\n\n`deny` wins over `allow` for a tool named in both, and empty lists read as \"no restriction\".\n\nThis is the selective, insert-aware counterpart to `ownsChildren`, which is all-or-nothing and clamps moves only. It is also the generic form of the Table tool's `restrictedTools`, whose enforcement is hard-wired to table cells.\n\nWithout it a container tool has to defend itself downstream: filtering `child.name` in render, keeping its CSS working when a foreign child appears, and migrating strays out of stored documents. On the React/Vue/Angular adapters, declare it in the block spec's `statics` bag like any other class static.",
...
export const Segments = createReactBlock({
  ...
  component: SegmentsCard,
      ...
      },
      {
        ...
        description:
          "An optional method on your tool that applies new data to the LIVE instance. Declare it, and `blocks.update()`, undo/redo and remote collaborative edits all reuse the block you already rendered instead of recomposing it.\n\nThere is no new tool instance and no new holder, so ephemeral state survives: an open menu, a scroll position, a framework component's local state. The adopted child holders and the caret survive too.\n\nWithout it, core destroys the block and builds a replacement. That is why a host calling `blocks.update()` on every keystroke used to watch a component-backed block go blank.\n\n- Return `false` when you cannot apply the data in place, and core falls back to the full recompose.\n  - The list tool does that for a style change, which needs a different DOM shape.\n- Returning `true` or nothing means the data was applied.\n- Throwing has the same effect as `false`: it is logged, then the block is recomposed.\n\nThe React/Vue/Angular block factories implement it for you, so adapter blocks take the in-place path automatically.",
...
// → true for a fresh editor holding one blank paragraph`,
      },
      {
        ...
        description:
          "Normalizes a whole loose backend DTO into the strict saved OutputData shape, exported from the main entry.\n\n- A nullish document becomes `{ blocks: [] }`.\n- `null` envelope fields (`time`/`version`) are dropped.\n- Each block is normalized too.\n  - A `null` or missing `data` becomes `{}`.\n  - A `null` or empty id is dropped, so a new one is generated.\n  - Nullish or empty hierarchy references (`parent: null`, `content: null`, `content: []`) are dropped as absent.\n\nUnlike a hand-written `blocks.map(...)` mapper, it keeps every passthrough field: `tunes`, real `parent`/`content` references, `indent`, and edit metadata. So hierarchy and tunes are never silently lost. It is idempotent: a strict document passes through unchanged.",
...
// → strict OutputData, safe to persist or diff — no blind \`as OutputData\` cast`,
      },
      {
        ...
        description:
          "Block-level counterpart of normalizeOutputData, exported from the main entry. It normalizes an array of loose wire blocks into the strict saved shape.\n\nA `null` or missing `data` becomes `{}`, a `null` or empty `id` is dropped so a new one is generated, and nullish or empty `parent`/`content` is dropped as absent. Every other field passes through untouched.\n\nUse normalizeOutputData when you hold the whole document envelope.",
...
// → OutputBlockData[] with tunes/parent/content/indent intact`,
      },
      {
        ...
        description:
          "A type helper, exported from the main entry. It lets a block-data shape declared with an `interface` fit the `data` slot.\n\nA TS `interface` has no implicit index signature, so it is not assignable to `Record<string, unknown>`. That is why `OutputBlockData<'task', TaskData>` fails to compile when `TaskData` is an interface.\n\n`BlokData<T>` re-projects `T` through a homomorphic mapped type. The compiler does treat that as having an implicit index signature, and every declared key keeps its precise type. You need no rewrite: an existing interface value is assignable to `BlokData<T>`, and a `type` alias already satisfies the slot on its own.",
...
const block: OutputBlockData<'task', BlokData<TaskData>> = {
  ...
  data: { title: 'Ship it', done: false },
      ...
      },
      {
        ...
        description:
          "Turns an ergonomic nested spec into the flat DFS pre-order `OutputBlockData[]` that Blok stores. It wires every `parent`/`content` link for you.\n\nExported from the main entry, along with its `BlockTreeSpec`, `BlockRunSpec`, `BlockTreeNode` and `FlattenTreeOptions` types. A spec node is `{ type?, data?, tunes?, id?, children? }`.\n\nIt is the pure counterpart of the live `blocks.insertTree()` mutation: the same DFS without an editor. So you can seed nested content (columns, tables, a whole document) without hand-authoring `parent`/`content` id arrays.\n\nEvery returned block has a resolved `id`, generated when the spec omitted one, so the array is safe to reference by id. Leaves omit the empty `content` array.\n\nContent that is already flat goes in as a run node, `{ blocks: [...] }`, at the root or as a child. A stored Blok document that a migration is splicing into a page is the usual case.\n\n- The run is spliced verbatim: ids, `data`, `tunes` and existing `parent`/`content` links are kept.\n- Only the blocks it left un-parented are re-parented onto the enclosing node.\n\nThat is the same rule `blocks.insertMarkdown()` applies to a converted run. Because nothing is re-derived, ids stay stable, so a migration can run in batches without duplicating blocks it already wrote.\n\n`options` takes `parentId`, the `parent` assigned to the root node or nodes, and `generateId`, the id generator for nodes without an explicit `id`. Pass a deterministic generator for reproducible output.\n\nReusing an explicit `id` within the spec throws, and so does passing a pre-flat block as a tree node, because its `parent`/`content` links would be dropped silently.",
...
const isPristine = equalsOutputData(saved, EMPTY_OUTPUT_DATA, {
  ignoreEmptyDefaultBlocks: true,
      ...
      },
      {
        ...
        description:
          "Maps a controlled `data` value to something render()/blocks.render() accepts, exported from the main entry. A whole-document `null`, which is a controlled \"clear to empty\", becomes `{ blocks: [] }`. Any real document passes through untouched.\n\nThe strict guard in render() reads `data.blocks` and would throw on `null`, so route a nullable controlled value through this first.",
...
await editor.blocks.render(toRenderableData(draft));`,
      },
      {
        ...
        description:
          "Creates a bounded window of recently emitted onSave payloads, so you can recognize controlled-`data` echoes. Exported from the main entry.\n\nDeduping against only the LAST emitted payload is not enough. A host that persists on save and refetches can hand back a STALE echo, an earlier save arriving after a newer one already replaced the baseline. Re-rendering that echo would clobber the caret and any content typed since.\n\nMatching is structural (equalsOutputData), so envelopes reshaped in transit (fresh `time`, stripped ids) still count as echoes.",
...
// before re-rendering incoming props: if (echoes.matches(next)) return;`,
      },
      {
        ...
        returnType: "OutputBlockData[]",
        description:
          "Migrate legacy or Editor.js-style blocks into Blok's hierarchical flat-with-references format, exported from the `@bloklabs/core/migrate` subpath. It is the same transform the renderer runs automatically at load.\n\nLegacy nested shapes (list items, toggle and callout children) explode into separate blocks linked by `parent`/`content`. `parent` is the saved-document field; `parentId` is the useBlocks BlockNode snapshot field.\n\nBlocks with no id are stamped with one. Already-hierarchical blocks pass through unchanged, so it is safe to run on current data and idempotent across repeated runs.\n\n`options` exposes the migration context.\n\n- `generateId` makes the pass PURE: migrate the same document twice and the outputs are equal.\n  - You need that to compare a stored doc against its migration, or to re-run migration per render without minting fresh ids.\n- `onLossyField` delivers every dropped field instead of dumping it to `console.warn`.\n- `rules` adds your own grammar entries.\n\n`migrateLegacyOutputData(data, options?)` is the envelope-preserving variant. `needsLegacyMigration(blocks, options?)` reports whether a migration would change anything. `matchLegacyRule(block, options?)` is the per-block primitive: it returns the entry claiming a single block (or `null`) without re-scanning the table for every block.\n\nEvery rules-taking entry point accepts either the options object or a bare `rules` array, so passing the array directly can't silently read as \"no rules\".",
        example: `import {
  migrateLegacyBlocks,
...
const entry = matchLegacyRule(stored.blocks[0]); // → { legacyType, targetType, … } | null`,
        note:
          "Some data shapes only a specific tool understands (a columns layout, a custom media envelope), and core's built-in migration can't read them. Give that tool a static `upgradeData(data)`, a pure function that returns the tool's current data shape.\n\nBlok runs it at load, while composing each stored block, before the tool is constructed. A hook that throws is caught, and the block loads with its stored data.",
      },
      {
        name: "migrations (config) & migrateOutputData(data, migrations)",
        returnType: "OutputData",
        description:
          "Declare per-type \"old data shape → new data shape\" rules from OUTSIDE the tool class. `upgradeData` must live inside a tool you own. `migrations` is a map keyed by block type that you pass in editor config. So you can migrate a third-party tool you don't control, or your own tool without editing (and re-shipping) its class.\n\nEach rule is a pure `(data) => data` transform. Return the input unchanged, or `undefined`, when the data is already current.\n\nBlok applies the rule at load, after the tool's own `upgradeData` and BEFORE format analysis. That way `dataModel: 'auto'` sees the post-migration shape, and an 'auto' round-trip can't quietly undo the migration by saving the old shape back. A rule that throws falls back to the stored data, never a blank editor.\n\nThe same map works offline: pass it to `migrateOutputData(data, migrations)` (or `migrateBlocks(blocks, migrations)`) from `@bloklabs/core/migrate` to batch-upgrade persisted records without opening an editor. All three framework adapters take it as the `migrations` prop or input.",
...
const upgraded = migrateOutputData(storedDocument, {
  myCard: (data) => ({ ...data, title: data.name }),
        ...
        note:
          "Rules must be pure and idempotent. They run on every load, including on already-current data.\n\n- Prefer `migrations` (config) for shapes a host decides from the outside.\n- Prefer a tool's own `upgradeData` for shapes only that tool understands.\n\nThey compose: `upgradeData` runs first, then the config `migrations` rule for that type.",
      },
      {
        name: "migrate(data, { migrations, rules, generateId, onLossyField })",
        returnType: "{ data: OutputData; report: MigrationReport }",
        description:
          "The composed entry point. It runs BOTH migration passes in the one correct order and reports what the migration cost.\n\n- Data rules (`migrations`) run first.\n- Then grammar rules (`rules`) restructure the tree.\n\nThat order is load-bearing. Data rules are keyed by block TYPE, and the grammar rewrites types (`linkTool` → `bookmark`) and explodes containers into many blocks. So a rule run after the grammar never fires, and the block stays silently unmigrated.\n\nData rules shape the grammar's input. The grammar owns the output shape for the types it rewrites.\n\nThe `report` names every field the mapping could not carry over (`lossyFields`) and every data rule that threw (`errors`). So a batch upgrade of persisted records is no longer silent about its own data loss.",
...
report.errors;      // [{ type: 'myCard', error }] — that block kept its stored data`,
      ...
      },
      {
        ...
        description:
          "Teach the migration machinery a legacy shape Blok doesn't know. A grammar entry is `{ legacyType, detect, expand, targetType, cardinality, contributesNesting, lossyFields, docNote }`.\n\nPassing entries via `rules` reuses the whole interpreter: recursion into container bodies, the orphan re-parenting invariant, 1:N splits, and id minting. You do not re-implement the dispatch loop around a data-only rule.\n\nHost entries are matched BEFORE the built-in table, so they can also override a built-in mapping. Unlike a `migrations` rule, an entry may change a block's `type` and emit several blocks.\n\n`expand(block, ctx, { siblings, index })` may also return `{ blocks, consumed }` to absorb the following `consumed` siblings. That is the shape flat-with-count legacy formats need, where a container stores its body as \"the next N blocks\". `consumed` is clamped to what remains, so a truncated document can't over-consume.\n\nRead `LEGACY_GRAMMAR` to introspect the built-in coverage.",
...
const rowBlocks = blocks.getChildren(databaseBlockId);`,
      },
      {
        ...
        returnType: "BlockNode | null",
        description:
          "Insert one block (type, data, parentId, position, tunes, id, focus/caret, replace). Use `replace: true` with a `position` that targets an existing block to replace that block instead of inserting beside it. That is a programmatic \"turn into\".\n\nReturns the created node, or null when rejected:\n\n- an unknown tool type\n- a dangling parentId\n- a `replace` whose target is missing\n\nAn explicit id that already exists is insert-if-absent. The call is atomic: one undo step.",
    ...
    methods: [
      {
        ...
        returnType: "string",
        description:
          "Render a saved document to semantic HTML. It is synchronous and DOM-free, so it is safe in Node, workers, and React Server Components.\n\nEvery inline-content field is sanitized against the composed allowlist before interpolation, and the URL scheme policy is identical to the editor's. Returns '' for empty or malformed documents (nullish input is tolerated).",
        params: [
          {
            name: "data",
          ...
          },
          {
            ...
            required: false,
            description:
              "viewSchema from defineBlokSchema. It carries a single composed baseSanitize, folded from the enabled INLINE TOOLS and TUNES, not from block tools' own static sanitize. That config is spread over the renderer's default inline allowlist, so inline content displays under the same composition that produced it.\n\nviewSchema.tools is carried for consumers, but the renderer does not read it. To control a custom block's markup, use renderers.",
          },
          {
          ...
          },
          {
            ...
            required: false,
            description:
              "Custom renderers for INLINE elements, keyed by lowercase tag name. They are the inline counterpart of renderers, for marks whose display is not their stored markup: an inline equation stores only its LaTeX source, a mention only an id.\n\nEach one runs after sanitization, over the elements that survived it, and REPLACES the element with what it returns. Return undefined to keep the element as sanitized, or '' to drop it.\n\nThe returned markup is inserted as-is and is NOT re-sanitized. That is the same trust contract as a block renderer's output, so it may carry markup the inline allowlist would strip (KaTeX spans, for instance).\n\nelement is { tag, attrs, html, text }. This affects rendered HTML only: blocksToPlainText reads a mark's stored source, not its rendering.",
          },
          {
          ...
          },
          {
            ...
            required: false,
            description:
              "Pure URL rewrite hook. It runs on every block URL (image/video/audio src, file/bookmark/embed href) and on every inline anchor href, so you can rewrite hrefs or route CDN image URLs.\n\nctx is { attr: 'href' | 'src', blockType?: string }, and blockType is undefined for inline anchors. The hook runs BEFORE the unsafe-scheme strip, so a rewrite can never re-introduce a javascript:/data: sink. Return '' to drop the URL.",
          },
          {
            name: "options.root",
            type: "boolean",
            ...
            default: "false",
            description:
              "Wrap the output in <div data-blok-interface=\"view\">. This is not cosmetic.\n\nThe scoped preflight and the token/colour layers key on the bare [data-blok-interface] attribute, so emitted classes compute differently without the wrapper and @bloklabs/core/view.css cannot reproduce the editor's appearance.\n\nIt is opt-in because it adds an element to existing output. <BlokView> stamps the attribute on its own wrapper, so React consumers never set this.",
          },
          {
      ...
      },
      {
        ...
        params: [
          ...
          },
          {
            ...
            required: false,
            description:
              "Also read the media text the default output leaves out, because the editor paints it as an attribute or does not paint it at all. That covers an image's alt, a video's or file's url, an embed's source, an audio track's title, artist and url, and a bookmark's description and url.\n\nEach one is appended after the block's visible label, one per line. It is off by default, so the default output stays exactly what a reader sees on screen.\n\nTurn it on for a search index, where alt text and a pasted URL are both things people search for.",
          },
        ],
        note:
          "There is no core \"document size\" helper, because the two sizes you might mean are measured differently.\n\n- `blocksToPlainText(data).length` is the visible content length, which is what a user typed.\n- `new TextEncoder().encode(JSON.stringify(data)).length` is the transport size in bytes, which is what a save or upload limit (e.g. 500KB) should check.\n\nUse the plain-text length for content rules and the JSON byte length for storage rules.",
...
// → '<b>bold</b>'`,
      },
      {
        ...
        description:
          "Extract the heading outline of a saved document, the source for a table of contents. It is synchronous and DOM-free. It walks the document in reading order (top-level blocks, then structural children), picks header blocks, and reduces each heading's inline HTML to plain text.\n\nEach item is { id?, level, text }. The block id drives anchor links and scroll targets, so no separate DOMParser pass is needed. Headings with empty text are skipped.",
...
// → [{ id: 'h1', level: 1, text: 'Getting Started' }, ...]`,
      },
      {
        ...
        description:
          "Repair in-document links whose target was lost during an import. HTML addresses its own sections by an id on the heading. Google Docs writes <h2 id=\"h.2y1ok8y7pef0\"> and links its table of contents to that fragment.\n\nA converter that mints its own block ids and drops the source ones leaves those links pointing at nothing. The link's text still names the heading, so this pass hands each dead fragment to the heading that text names, as HeaderData.anchor.\n\nBecause it writes content, it guesses as little as possible.\n\n- It takes only headings with no anchor yet.\n- It needs an exact text match. Markup, entities and whitespace are normalized away first, punctuation is not.\n- It acts only when exactly one heading and one fragment claim each other.\n\nAnything less certain is left alone and listed in report.skipped. Running it twice changes nothing further.\n\nCall it yourself as a one-off upgrade: it is not part of the automatic load path. It is DOM-free, so it runs in a Node script over stored records. Migrate legacy data first.",
...
await save(data);`,
      },
      {
        ...
        description:
          "Resolve a tools/inlineToolbar/tunes config into one shared schema. It is pure and module-scope-safe: call it at module scope and import the result everywhere.\n\nSpread editorConfig into new Blok(...) and pass viewSchema to the view functions. This guarantees documents are displayed under the SAME sanitize composition that produced them, instead of two configs drifting apart.\n\nThe guarantee is per composition: if you change the inline-tool set at runtime with tools.setInlineToolbar, recompose the schema from the current config. Options that don't participate in schema resolution (link, i18n, data, …) pass through editorConfig untouched.",
...
const html = blocksToHtml(savedData, { schema: schema.viewSchema });`,
      },
      {
        ...
        description:
          "Fold an ordered list of sanitize configs with the editor's exact merge semantics: a later-wins Object.assign, inline tools first, then tunes. Function rules are carried by reference. Rules for the same tag are REPLACED, never deep-merged.\n\nThis is the same fold defineBlokSchema uses to build viewSchema.baseSanitize, exposed for hand-built lists. It is exported from both @bloklabs/core and @bloklabs/core/view.",
...
// → { b: {}, i: {}, a: { href: true } }`,
      },
      {
        ...
        returnType: "ViewNode[]",
        description:
          "Render to a framework-agnostic JSON tree instead of an HTML string. Each node is { tag, attrs, children } or { text }, with the same options and sanitization pipeline as blocksToHtml. This is what the React bindings map to real elements.\n\nIt is experimental: the shape is not frozen until a second framework adapter consumes it, so it may change in a minor release.",
...
// → [{ tag: 'p', attrs: {}, children: [{ text: 'Hello' }] }]`,
      },
      {
        ...
        returnType: "Promise<string>",
        description:
          "Render a LaTeX string to HTML with the KaTeX build Blok already bundles (its code tool, equation inline tool and markdown importer all use it).\n\nIt is hardened for untrusted input.\n\n- trust: false forbids the markup-injecting commands (\\href, \\includegraphics, \\html*).\n- maxExpand caps macro expansion.\n- maxSize caps element sizing.\n- throwOnError: false renders malformed math as escaped source instead of failing the document.\n\nUse it instead of adding katex as your own dependency, which is a second copy of the library and a second, unaudited option set.\n\nKaTeX is imported lazily on the first call. With no document present (SSR, workers) the stylesheet injection is skipped and you include katex.min.css yourself, and the markup is identical.\n\nFor inlineRenderers use createLatexRenderer, which is synchronous.",
...
const html = await renderLatex('c = \\\\pm\\\\sqrt{a^2 + b^2}', { displayMode: false });`,
      },
      {
        ...
        description:
          "Load KaTeX once and get back a synchronous LaTeX renderer. That is the form inlineRenderers needs, since it replaces an element with the string it returns, and a promise there stringifies as [object Promise].\n\nIt is shaped as \"await the loader, get the renderer\", so there is no call order to get wrong: the renderer cannot exist before KaTeX is ready. It uses the same hardened options as renderLatex.\n\nThis is what lets a display surface render equations through blocksToHtml/BlokView instead of booting a read-only editor for them.",
  ...
  inlineRenderers: {
    span: ({ attrs }) => attrs['data-latex'] === undefined
      ...
      },
      {
        ...
        description:
          "The React display component from @bloklabs/react. It renders a saved document inside a single <div> wrapper: no editor instance, no chrome, no async, no effects, and never dangerouslySetInnerHTML (content is mapped from the sanitized view tree to real React elements).\n\nThe wrapper always carries data-blok-interface=\"view\", which is what makes the emitted classes compute as they do in the editor. It is written BEFORE the divProps spread, so a caller can override it. Never override it to \"blok\", which carries all: initial !important and would block host typography.\n\nThis is the read-only path to reach for at display-only call sites, instead of <BlokEditor readOnly>: it costs no editor bundle, has no ready latch, and renders identically under SSR.\n\nIts props API is stable. Only the raw ViewNode tree it maps from (via blocksToViewNodes) stays experimental, and using BlokView never exposes you to it.",
        params: [
          {
            name: "data",
            type: "OutputData | LooseOutputData | null | undefined",
  ...
  return (
    ...
    />
      ...
      },
      {
        ...
        returnType: "ReactNode",
        description:
          "The wrapper-free form of BlokView. It returns a Fragment of the block elements with no extra <div>, for slots where a wrapper is invalid or unwanted: checkbox labels, table cells, headings. It is synchronous and effect-free (SSR-safe), memoized on the data reference and the individual option values.\n\nIt takes the same options as blocksToHtml, with two exceptions.\n\n- root is ignored, since emitting no wrapper is the hook's contract.\n- classes defaults to false here, while it defaults to true in <BlokView>, which owns a wrapper.\n\nPassing root: true type-checks and silently does nothing. When you need view.css to paint, wrap the returned Fragment yourself in an element carrying data-blok-interface=\"view\".",
