...
| `callout` (legacy `body`/`variant` shape) | Auto-migrated at runtime **+** codemod | Legacy `{ title, body, variant, emoji }` → flat `{ emoji, textColor, backgroundColor }`; a non-empty `title` becomes the first child paragraph, then body blocks flatten into parented children. |
...
| `table` (HTML-string cells) | Auto-migrated at runtime **+** codemod | Each non-empty cell becomes a `{ blocks: [ids] }` cell-ref plus child blocks (`parent` = table id). In the editor and `migrate()` with a DOM, lines and lists in the cell become separate `paragraph`/`list` blocks; the codemod (no DOM) keeps the cell as one `paragraph`; empty cells become `{ blocks: [] }`. `withHeadings` is preserved. Cells already in block-ref shape pass through. |
