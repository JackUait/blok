...
export const serverLimits: ServerLimit[] = [
  ...
  },
  {
    ...
    body: 'Blok draws presence itself: small avatars for the people in the document, and a colored cursor showing exactly where each of them is, with their name on a small flag beside it. The flag fades a couple of seconds after that person stops moving, leaving the colored line behind — otherwise a document with four people in it is a wall of name tags. Both read the display identity you pass in the editor’s collaboration option — a name, and an optional color used for that person’s avatar and cursor. Leave it out and the person still shows up, just without a name on them. This is display only, and independent of the separate user option that stamps who last edited a block: one answers "whose cursor is that", the other answers "who gets the credit for this edit". Set either, both or neither. One thing the cursor does not do yet is shade the text somebody else has selected; that is a later phase.',
  },
  {
