...
export const serverLimits: ServerLimit[] = [
  ...
  },
  {
    ...
    body: 'The cap above counts one person; this ceiling counts everybody together, in every auth mode: with --collab on, the standalone service holds at most 1,024 open collaboration connections per process — a limit of the web server it runs on (Kestrel), there so a flood of connections that are opened and then simply held cannot use up every connection the process can hold. Past it, a new connection is refused until one of the open ones closes. Inside your own ASP.NET app the service sets no such ceiling — Kestrel’s own default is no limit — so it is whatever your Kestrel configuration says (MaxConcurrentUpgradedConnections).',
  },
  {
    id: 'collab-scale-out',
  ...
  },
  {
    ...
    body: 'A document relays presence — the avatars, and the cursor showing where each person is typing — for up to 256 people at once. Past that the presence updates are dropped instead of passed on, and the service writes a line to its log saying so. Editing is untouched: everyone keeps typing, and every change still merges and reaches your records as usual. So if a document ever gets that crowded, what breaks is people seeing each other, not people working — this is a presence-only degradation, not a limit on how many may edit one document. There is no setting for the number; it is fixed inside the service.',
  },
  {
  ...
  {
    id: 'service-account-home',
    title: 'A service account with no home directory needs DOTNET_BUNDLE_EXTRACT_BASE_DIR',
    body: 'The release archive ships the service as a single file that unpacks itself on first start, into a folder under the home directory of the account running it. A service account with no home directory has nowhere to unpack, so the process dies before the service starts, complaining that the default extraction directory is not accessible: set DOTNET_BUNDLE_EXTRACT_BASE_DIR to a directory that account can write — and not the uploads directory, because everything there is served. The Docker image already sets it (/var/tmp/.net).',
  },
];
